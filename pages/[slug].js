...
export async function getStaticPaths() {
  const paths = posts.map(p => ({ params: { slug: String(p.slug) } }))
...
export async function getStaticProps({ params }) {
  ...
  const related = posts
    ...
    .map(p => {
      ...
      return { id: p.id, slug: p.slug, title: p.title, date: p.date, category: p.category, thumbnail: relThumb }
    })
...
function RelatedCard({ post }) {
  ...
  return (
    <a href={'/' + post.slug + '/'} className="related-card" style={{
