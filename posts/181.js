const post = {
  id: 181,
  sections: [
    {
      type: 'intro',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>2024년 12월 11일 개봉예정영화 더 크로우( The Crow ) 관람평 후기와 영화 제목 뜻 의미 해석입니다.</b></span></p><p>현재 해외에서 로튼토마토 평론가 관객 평가가 엇갈리며 극단적인 차이를 보이고 있습니다.</p><p>과연 어떠한 점이 이러한 평가를 가져왔는지 확인해 보도록 하겠습니다.</p>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post181_img1.jpg', alt: '더 크로우 The Crow', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/netflix-movie-recommend-guide-top-10/',
      text: '넷플릭스 영화 추천 2024년 9월 인기 작품 순위 TOP 10 총정리!'
    },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '더 크로우 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>더 크로우 로튼 토마토 평론가 평점입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post181_img2.jpg', alt: '더 크로우 ( The Crow ) 영화 관람평 포토 정보 결말 후기 뜻 의미 해석 <평론가 관객 엇갈린 평가>', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><figure><figcaption>더 크로우 로튼 토마토 평론가 평점</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 22%, 관객 평점 63%의 더 크로우입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>20점대라는 압도적으로 낮은 평론가 평점과 높진 않지만 60점대의 관객 평점을 기록하였습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>관객 평점도 그다지 높은 평점은 아니지만 어떠한 점이 이러한 결과를 가져왔는지,</p>'
    },
    {
      type: 'body',
      html: '<p>평론가들의 관람평 결말후기 확인해 보겠습니다.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;>영화 제목의 뜻 의미는 포스팅 하단에서 확인 가능합니다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '더 크로우 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>더 크로우 ( The Crow ) 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post181_img3.jpg', alt: '더 크로우 로튼 토마토 평론가 평점', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><b>Matt Brunson - 평점: 1/4</b><br>이 영화는 완전히 끔찍하다.</br></p>'
    },
    {
      type: 'body',
      html: '<p><b>Allison Rose - 평점: 2.5/5</b><br/>빌 스카스가르드의 연기는 빛나지만, 약한 캐스트와 허술한 플롯이 이 영화 ‘더 크로우’의 완성도를 떨어뜨린다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Jack Bottomley - 평점: 2/5</b><br/>이 영화는 몇 가지 폭력적인 장면을 제외하고는 예술적으로 만족스럽지 않으며, 감정적으로도 기대 이하이다. 안타깝게도 실패작이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mini Anthikad-Chhibber - 평점 없음</b><br/>시각적으로 매력적이고 그루 비한 사운드를 자랑하지만, 어색하고 평면적인 대사와 불필요한 고어 장면들로 인해 이 리부트는 쉽게 잊히는 작품이 될 것이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Sean P. Means - 평점: 2.5/4</b><br/>‘더 크로우’은 불안정하게 날아가며, 속도 부족으로 결국 너무 익숙한 결말로 향한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Tim Cogshell - 평점 없음</b><br/>이 영화는 새롭고 신선한 배우들로 채워졌을 뿐, 그 이상으로 자랑할 만한 점은 없다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Kat Hughes - 평점: 4/5</b><br/>빌 스카스가르드와 FKA 트위그의 열정적인 연기, 그리고 스토리텔링과 사운드트랙의 강렬함이 ‘더 크로우’를 비극적인 이야기로 승화시켜 모든 관객을 매혹시킬 준비가 되어 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Austin Burke - 평점: 3/10</b><br/>영화의 첫 번째 액트는 캐스트가 마치 수면 상태로 연기하는 것 같아 지루하게 느껴지며, 그 이후로도 리듬을 찾지 못한다. 가장 중요한 감정적인 순간조차 제대로 전달되지 않는다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Sarah Vincent - 평점: 6/10</b><br/>리부트 치고는 ‘더 크로우’은 좋은 앙상블 캐스트와 분위기 덕분에 편집과 이야기의 갭을 메워주지만, 여전히 전체적으로 완성되지 않은 인상을 준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Dennis Schwartz - 평점: C+</b><br/>‘더 크로우’은 1994년 원작 컬트 클래식의 어두운 리부트 혹은 재구상이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Cris Kennedy - 평점: 2/5</b><br/>스타일은 흥미롭지만, 영화는 전반적으로 주제나 메시지에서 크게 벗어나며, 그다지 의미 있는 내용을 전달하지 못한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Tony Baker - 평점: 2/5</b><br/>이 영화의 액션은 전혀 감동을 주지 못했다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Nicolás Delgadillo - 평점 없음</b><br/>컬트 클래식 원작에 비해 부족한 부분이 있지만, 그래픽 노블을 기반으로 한 이 최신 버전은 그 나름대로의 장점이 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Wenlei Ma - 평점: 2/5</b><br/>많은 영화들이 ‘나쁜’ 영화라도 흥미로운 점이 있지만, ‘더 크로우’은 그럴 여지도 없다. 진지하지만 과도하게 진지하다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '더 크로우 뜻 의미 해석',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>더 크로우 뜻 의미 해석입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post181_img11.jpg', alt: '더 크로우 관람평', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post181_img10.jpg', alt: '더 크로우 관람평', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post181_img9.jpg', alt: '더 크로우 관람평', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post181_img8.jpg', alt: '더 크로우', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post181_img7.jpg', alt: '더 크로우', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post181_img6.jpg', alt: '더 크로우', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post181_img5.jpg', alt: '더 크로우', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post181_img4.jpg', alt: '더 크로우', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>더 크로우 The Crow<br/></span>까마귀</b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>&quot;더 크로우(The Crow)&quot;라는 제목은 영화의 주인공 에릭 드레이븐이 <span style=&quot;background-color: #9feec3;&quot;><b>자신의 죽음 이후 초자연적 존재로 부활하며 복수를 시작하는 중심 상징인 &quot;까마귀&quot;를 뜻</b></span>합니다.</p>'
    },
    {
      type: 'body',
      html: '<p>까마귀는 이 영화에서 그의 부활과 복수의 원동력, 그리고 그가 죽음과 생명 사이를 넘나드는 존재임을 나타냅니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>이 새는 고딕적이고 신비로운 분위기를 강조하며, 주인공의 초자연적 여정을 상징적으로 뒷받침</b>합니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화  로튼 토마토 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' }
  ]
}

module.exports = post
