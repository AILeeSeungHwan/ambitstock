const post = {
  id: 151,
  sections: [
    {
      type: 'intro',
      html: '<p>라팔마 넷플릭스 드라마 평점 뜻 스포포함 결말 후기 관람평 해외 반응 < 재난 시리즈 추천 하나? ></p>'
    },
    {
      type: 'image',
      src: '/images/post151_thumb.svg',
      alt: '라팔마 넷플릭스 드라마 평점 뜻 스포포함 결말 후기 관람평 해외 반응 < 재난 시리즈 추천 하나? >',
      caption: '라팔마 넷플릭스 드라마 평점 뜻 스포포함 결말 후기 관람평 해외 반응 < 재난 시리즈 추천 하나? >'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '라팔마 평점 뜻 스포포함 결말 후기 관람평 해외 반응 알아보자',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '라팔마 해외 평점',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>라팔마의 로튼 토마토 평론가, 관객 평점입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post151_img1.jpg', alt: '라팔마', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 x, 관객 평점 36%로 낮은 점수를 기록하였습니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>최대 영화 드라마 리뷰 사이트에서도 <b>6.4점으로 낮은 점수를 기록하였지만 인기 순위는 105위로 높은 순위</b>를 기록중입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>로튼 토마토보다는 상대적으로 높은 평점임을 알 수 있습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분의 좋지 못한 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '라팔마 스포포함 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p><b>7/10 (오리지널 스코어: 7/10)</b><br>매우 훌륭했으나, 나머지 10%는 확실하지 않았다. 이 작품은 정말 좋았다. 경치와 촬영 기법은 훌륭했고, 애니메이션의 퀄리티도 매우 사실적이었다. 긴장감은 끊임없이 지속되었으며, 어느 순간부터는 스위치를 켠 것처럼 계속해서 긴장을 놓지 않았다.</br></p>'
    },
    { type: 'image', src: '/images/post151_img2.jpg', alt: '라팔마 평점', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>나는 멈추지 못하고 한 번에 전부 봤다. 그러나 나에게 정말 이상하게 느껴졌던 점은 작은 플롯의 구멍들과 조금은 유치한 청춘 로맨스였다. 이 부분은 약간 어색했고, 전체적으로 탄탄한 TV 작품에 조금 신뢰를 떨어뜨렸다. 왜 그런 방향으로 갔는지 이해할 수 없었고, 몇몇 장면에서는 화면을 보며 \'으으\' 하기도 했다.</p>'
    },
    {
      type: 'body',
      html: '<p>몇몇 다른 어처구니없는 장면들이 있었지만, 그 10%를 무시할 수 있다면 여전히 훌륭한 작품이었다. 진심으로 정말 재미있었고, 누구에게나 추천할 만하다. 사실 이 작품을 7/10으로 평가한 것이 조금 미안했다. 그 작은 단점들만 없었다면 8점 이상을 줄 수 있었을 것이다. 메리 크리스마스!</p>'
    },
    {
      type: 'body',
      html: '<p><b>6/10 (오리지널 스코어: 6/10)</b><br/>나는 자연 재해에 관한 영화를 좋아하지만, 그에 플롯 아머를 섞으면... <br/>SPOILER<br/>나는 아내와 함께 &quot;그녀가 살아남으면 이 영화에 대한 리뷰를 남기겠다&quot;고 약속했다. 먼저 이 영화에는 아름다움이 있다. 예를 들어, 풍경은 정말 놀랍고, 여기저기 교육적인 요소도 있으며, 자연 재해의 효과는 훌륭했다. 그런데 왜 그런지 모르겠다... 정말로, 매력적인 캐릭터를 만들기가 그렇게 어려운 걸까? 내가 가장 좋아했던 캐릭터는 알루미늄 포일로 싸여진 두 남자였다. 그들은 마지막 순간을 잘 보내는 법을 알고 있었다.</p>'
    },
    {
      type: 'body',
      html: '<p><br/>오해하지 말길, 나는 해피 엔딩을 좋아한다. 하지만 3일 만에 만난 &quot;여자친구&quot;를 가족보다 우선시하는 것과, 비행기를 반으로 나누는 쓰나미에서 살아남는 것은 너무 터무니없다. 마지막 순간에 나는 그 여자가 \'파이널 데스티네이션\'처럼 비행기에서 죽지 못하고 결국 죽을 것 같았다. 하지만 그녀의 플롯 아머는 너무 강력해서 사신도 그녀에게 머리만 작은 상처를 주었다. 잘 했다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>7/10 (오리지널 스코어: 7/10)</b><br/>흥미로운 작은 판타지, 하지만... <br/>SPOILER<br/>아름다운 장소에서 촬영된 이 미니 시리즈는 정말 신선하고, 훌륭한 촬영과 탄탄한 연출을 자랑한다.</p>'
    },
    {
      type: 'body',
      html: '<p>라팔마에서 촬영된 장면들은 가족이나 친구들과 함께 보기에도 충분히 재미있다.</p>'
    },
    {
      type: 'body',
      html: '<p><br/>하지만 몇 가지는 여전히 나를 불편하게 만들었다. &quot;Charlie&quot;의 서브플롯은 전혀 필요 없었고, 딸이 내린 결정들은 너무 멍청했고 그들의 &quot;관계&quot;는 강제로 억지로 이어진 것처럼 보였다.</p>'
    },
    {
      type: 'body',
      html: '<p>나는 이 작품을 8점 또는 9점으로 평가하고 싶었지만, 그들이 &quot;역사상 가장 큰 자연 재해&quot;에서 살아남은 방식은 불가능했다.</p>'
    },
    {
      type: 'body',
      html: '<p>해안 가까운 곳에서 살아남는 것은 말이 안 된다. 그래서 7/10이 내가 줄 수 있는 최고 점수였다.</p>'
    },
    {
      type: 'body',
      html: '<p>결론적으로, \'라팔마\'는 즐거운 작은 TV 미니 시리즈이고, 일부 장면에서는 머리를 잠시 끄고 보면 훨씬 더 좋은 시간을 보낼 수 있을 것이다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '라팔마 뜻 줄거리',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p>넷플릭스의 노르웨이 드라마 <i>\'라팔마\'</i>는 스페인 카나리아 제도의 라팔마 섬을 배경으로 한 재난 시리즈입니다.</p>'
    },
    { type: 'image', src: '/images/post151_img8.jpg', alt: '라팔마', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post151_img7.jpg', alt: '라팔마', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post151_img6.jpg', alt: '라팔마', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post151_img5.jpg', alt: '라팔마', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post151_img4.jpg', alt: '라팔마', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post151_img3.jpg', alt: '라팔마 평점', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>이 작품은 총 4부작으로 구성되어 있으며, 휴가 중인 노르웨이 가족이 화산 폭발의 위협에 직면하는 스토리이며, 한 젊은 연구원이 화산 폭발이 임박했다는 불길한 징후를 발견하면서 가족은 재앙의 한가운데로 내몰리게 됩니다.</p>'
    },
    {
      type: 'body',
      html: '<p>초반에는 인물들 간의 관계와 감정에 집중하며, 후반부로 갈수록 재난 상황의 긴박함이 부각됩니다. CG는 과하지 않지만 자연스럽게 표현되어 몰입감을 높였습니다. 자연 재해와 인간의 대응을 현실적으로 그려내며, 재난물 특유의 과학적 설명이 포함되어 있습니다. 또한, 다양한 인간 관계와 갈등을 통해 드라마적 요소를 풍부하게 담고 있죠. 동성 커플의 이야기도 포함되어 있어 현대 사회의 다양한 모습을 반영하려는 시도가 엿보입니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>현재 넷플릭스에서 시청할 수 있으며, 재난물 장르를 선호하는 시청자들에게 추천할 만한 작품입니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
