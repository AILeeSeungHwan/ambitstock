const post = {
  id: 118,
  sections: [
    {
      type: 'intro',
      html: '<p>고스트캣 앙주 개봉! 관람평 포토 영어 제목 뜻 평점 < 로튼 평론가 후기 ></p>'
    },
    {
      type: 'image',
      src: '/images/post118_thumb.svg',
      alt: '고스트캣 앙주 개봉! 관람평 포토 영어 제목 뜻 평점 < 로튼 평론가 후기 >',
      caption: '고스트캣 앙주 개봉! 관람평 포토 영어 제목 뜻 평점 < 로튼 평론가 후기 >'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '일본 프랑스 애니메이션 영화 고스트캣 앙주 개봉',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>일본과 프랑스의 애니메이션 영화 고스트캣 앙주가 2025년 1월 22일 개봉합니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post118_img1.jpg', alt: '고스트캣 앙주', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>이번 포스팅에서는 <span style=&quot;background-color: #9feec3;&quot;><b>영화 제목인 고스트캣 앙주의 영어 뜻 해석, 그리고 해외 로튼 토마토 평론가 반응 결말 후기를 포토와 함께</b></span> 알아보도록 하겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post118_img2.jpg', alt: '고스트캣 앙주', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/movie-drama-recommend-netflix/',
      text: '볼만한 영화 드라마 추천 넷플릭스 시청가능 <이동진 박평식 한줄평 추천작품>'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/movie-recommend-critic-masterpiece/',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '고스트캣 앙주 줄거리',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>꾹꾹이 알바하는 고양이 실존?! <br>귀여운 동안 외모지만 37세다냥~</br></span></b></span></blockquote>'
    },
    { type: 'image', src: '/images/post118_img3.jpg', alt: '고스트캣 앙주', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><br>한적한 시골 마을에 위치한 ‘소세지절’. 아빠 ‘테츠야’는 엄마 기일 전까지 돌아온다는 말을 남기고 떠나고, 11세 소녀 ‘카린’만 혼자 남는다. 그런 ‘카린’의 무료한 일상에 37살 고양이 요괴 ‘앙주’가 등장한다.</br></p>'
    },
    {
      type: 'body',
      html: '<p>귀차니즘 아재 고양이 ‘앙주’와 한집살이를 시작한 까칠한 소녀 ‘카린’. 투닥거리는 사이 미운 정, 고운 정 다 들어버린 둘은 ‘카린’이 그리워하는 엄마를 만나기 위해 저승으로 함께 떠나게 되는데... 37살 아재 고양이 요괴 ‘앙주’와 시니컬한 11살 젠지 소녀 ‘카린’의 아주 특별한 저세상 여행이 시작된다!</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '고스트캣 앙주 영어 제목 뜻',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<blockquote><b><span style=&quot;color: #781b33;&quot;><span style=&quot;font-family: \'Noto Serif KR\';&quot;>고스트캣 앙주 (Ghost Cat ANZU)<br/></span>고양이요괴 앙주</span></b></blockquote>'
    },
    { type: 'image', src: '/images/post118_img4.jpg', alt: '고스트캣 앙주 평점', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>영화 제목 고스트캣 앙주의 영어제목은 Ghost Cat ANZU입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>고스트는 영어로 귀신이지만 영화의 주제와 일본이라는 문화 특성을 반영하여 요괴로 해석이 가능하고, </p>'
    },
    {
      type: 'body',
      html: '<p>캣은 고양이, 그리고 앙주는 영화 주인공 고양이인 \'앙주\'라는 캐릭터를 의미합니다.</p>'
    },
    {
      type: 'body',
      html: '<p>결론적으로 영어 제목, <span style=&quot;background-color: #9feec3;&quot;><b>영화 제목 고스트캣 앙주를 해석하면 \'고양이요괴 앙주\'라 읽으시면 되겠습니다.</b></span></p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '고스트캣 앙주 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>고스트캣 앙주 로튼 토마토 평론가 평점입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post118_img5.jpg', alt: '고스트캣 앙주', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 65%, 관객 평점 86%입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>평론가 평점은 60점 중반대로 낮은 평점을 기록하였지만, 관객 평점은 86점을 기록하며 두 그룹의 상반된 평가를 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 평론가들의 낮은 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '고스트캣 앙주 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>고스트캣 앙주 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post118_img11.jpg', alt: '고스트캣 앙주 포토', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post118_img10.jpg', alt: '고스트캣 앙주 포토', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post118_img9.jpg', alt: '고스트캣 앙주 포토', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post118_img8.jpg', alt: '고스트캣 앙주', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post118_img7.jpg', alt: '고스트캣 앙주', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post118_img6.jpg', alt: '고스트캣 앙주', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><b>Charles Solomon - 평점 없음</b><br/>매우 답답한 영화로, 무엇을 하고 싶은지 제대로 정하지 못한 영화이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Jana Monji - 평점 3/5 (오리지널 스코어)</b><br/>아카시 이마시로의 만화를 바탕으로 한 이 영화는 작은 마을에서 일본 민속의 이상한 존재들이 일상 속에 존재하는 모험과 우정에 대한 불균형한 이야기이다. (제목이 기대하게 만드는 것과는 달리 사실상 유령은 아니다.)</p>'
    },
    {
      type: 'body',
      html: '<p><b>Morris Yang - 평점 없음</b><br/>‘고스트 캣 앙주’는 기묘하고 조용히 전염성이 있는 작품으로, 엉뚱한 즉시성이 느껴진다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Emma Vine - 평점 없음</b><br/>결과물이 표면적인 영역에 머물러 있지만, 앙주는 응원하고 싶은 매력적인 캐릭터로, 무엇보다 90분 동안 장난스럽고 엉뚱한 재미를 준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Cody Dericks - 평점 5/10 (오리지널 스코어)</b><br/>케이이치 스즈키의 기운 넘치는 음악은 영화의 이야기에서 부족한 일관성을 제공하지만, 그 외의 부분에서는 부족함이 많다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Maxance Vincent - 평점 2.5/5 (오리지널 스코어)</b><br/>상상력은 넘치지만, 스토리텔링의 한계에 부딪힌 작품이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mark Dujsik - 평점 2.5/4 (오리지널 스코어)</b><br/>이야기가 이상한 방향으로 흐를 때, 내러티브는 중요한 것에 대한 감각을 잃고, 이상해지며 마치 그 자체를 위한 이상함처럼 느껴진다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Alan French - 평점 5/10 (오리지널 스코어)</b><br/>모든 애니메이션을 소비하는 관객이라면 ‘고스트 캣 앙주’를 즐겁게 볼 수 있을 것이다. 영화는 반복적이고 다소 지루하지만, 후반부에서 충분히 재미를 찾을 수 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>David Ehrlich - 평점 B (오리지널 스코어)</b><br/>‘고스트 캣 앙주’는 때때로 너무 편안해 보일 수 있지만, 상상력의 범위와 그 상상력이 세계를 그리는 방식에는 특별한 힘이 있다. 그 세계는 동시에 제한적이면서 무한한 느낌을 준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Daniel Kurland - 평점 7/10 (오리지널 스코어)</b><br/>‘고스트 캣 앙주’는 초보 기블리 작품과 비슷한 느낌을 준다: 즐겁고 사랑스럽고 시각적으로 화려하지만, 내러티브적으로는 공허하며 주제를 진정으로 발전시킬 수 있는 틀이 부족하다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Avi Offer - 평점 없음</b><br/>조금은 유쾌하고 달콤하며 재미있지만, 집중되지 않고 결국 잊힐 만한 영화이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Robbie Collin - 평점 4/5 (오리지널 스코어)</b><br/>영화는 믿을 수 없을 정도로 재미있고, 대체로 가족 친화적이지만, 정말 존재해서는 안 될 것 같은 느낌이 든다: ‘센과 치히로의 행방불명’을 비즈 만화로 상상해 보라.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Douglas Davidson - 평점 4/5 (오리지널 스코어)</b><br/>이 영화는 가슴을 열고, 이 세계를 부드럽게 대하도록 관객에게 부탁하며, 그 시간이 충분히 가치 있게 느껴지도록 약속한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Jara Yáñez - 평점 없음</b><br/>영화의 대담함이 형식적이나 논쟁적인 면에서 두드러지지 않는 점이 아쉽다. [전체 리뷰는 스페인어로 작성됨]</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 고스트캣 앙주 로튼 토마토 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
