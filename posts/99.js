const post = {
  id: 99,
  sections: [
    {
      type: 'intro',
      html: '<p><b>애니메이션 영화 반지의 제왕: 로히림의 전쟁 (영어제목 : The Lord of the Rings: The War of the Rohirrim ) 이 2025년 1월 25일  개봉합니다.</b></p><p>이번 포스팅에서는 역대 최고의 시리즈로 뽑히는 <span style=&quot;background-color: #9feec3;&quot;><b>영화 반지의 제왕의 애니메이션 영화, 로히림의 전쟁 해외 평점과 후기를 알아보고 볼만한 작품인지 확인</b></span>해 판단할 수 있도록 도와드리겠습니다.</p><div><center><a href=&quot;https://search.naver.com/search.naver?sm=tab_hty.top&amp;where=nexearch&amp;ssc=tab.nx.all&amp;query=반지의+제왕+로히림의+전쟁+예매&amp;oquery=서바이브+정보&amp;tqi=iHth5lpzL8wssTfAvudssssssMl-116987&quot; title=&quot;반지의 제왕: 로히림의 전쟁 보러가기&quot;><button class=&quot;btn-hover color-8&quot;>반지의 제왕: 로히림의 전쟁 바로 보러 가기👉</button></a></center></div>'
    },
    {
      type: 'image',
      src: '/images/post99_thumb.svg',
      alt: '반지의 제왕: 로히림의 전쟁 관람평 줄거리 결말 <평론가 관객 엇갈린 평점>',
      caption: '반지의 제왕: 로히림의 전쟁 관람평 줄거리 결말 <평론가 관객 엇갈린 평점>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post99_img1.jpg', alt: '반지의 제왕: 로히림의 전쟁', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/netflix-movie-recommend-release-upcoming-guide/',
      text: '2025년 넷플릭스 한국 영화 추천 개봉예정작 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/movie-recommend-critic-masterpiece/',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '반지의 제왕: 로히림의 전쟁 줄거리',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p>‘반지 전쟁’ 200년 전, 전설로 남을 거대한 전투가 펼쳐진다.</p>'
    },
    { type: 'image', src: '/images/post99_img2.jpg', alt: '반지의 제왕: 로히림의 전쟁', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>로한의 전설적인 왕, 무쇠주먹으로 불리는 ‘헬름’은 공주 ‘헤라’를 아들 ‘울프’와 결혼시키려는 웨스트마크 영주 ‘프레카’와의 결투 도중 뜻밖의 사고로 그를 죽이고 만다. 아버지의 죽음에 분노한 ‘울프’는 복수를 다짐하며 모습을 감춘다.</p>'
    },
    {
      type: 'body',
      html: '<p>시간이 흘러 ‘울프’는 무자비한 던랜드인들을 이끌고 로한을 공격한다. 이에 맞서 ‘헬름’과 그의 아들 ‘할레스’, ‘하마’, 그리고 로한의 기마대 로히림은 필사적으로 싸우지만, 영주들의 배신으로 인해 전세가 불리해지고 결국 나팔 산성으로 퇴각하게 된다.</p>'
    },
    {
      type: 'body',
      html: '<p>‘헬름’은 두 아들인 ‘할레스’와 ‘하마’를 모두 잃고, ‘헤라’와 함께 나팔 산성에 고립된다. 하지만 ‘울프’와 대규모 던랜드 군대의 포위 속에서도, 그들은 로한의 생존을 걸고 마지막 전투를 준비하며 결전을 향해 나아간다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '반지의 제왕: 로히림의 전쟁 영어제목 뜻 해석',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p>영화 ‘반지의 제왕: 로히림의 전쟁(The Lord of the Rings: The War of the Rohirrim)’의 제목은 로한 왕국의 기마 전사들, 즉 로히림(Rohirrim)의 용맹과 희생을 중심으로 한 이야기를 담고 있습니다.</p>'
    },
    { type: 'image', src: '/images/post99_img3.jpg', alt: '반지의 제왕: 로히림의 전쟁', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>반지의 제왕 세계관 속 로한 왕국의 역사적인 전투를 조명하며, <b>\'그들의 영광스러운 전투와 전설로 남을 헬름 왕의 이야기\'</b>를 강조합니다.</p>'
    },
    {
      type: 'body',
      html: '<p>영화의 줄거리에서 로히림은 웨스트마크 영주 프레카와 그의 아들 울프를 중심으로 벌어진 내란과 외세의 침략으로 인해 절체절명의 위기를 맞이합니다. 헬름 왕은 이 모든 역경 속에서 나팔 산성으로 물러나 로한의 마지막 방어선을 구축하고, 로한의 생존을 위해 목숨을 건 전투를 벌입니다.</p>'
    },
    {
      type: 'body',
      html: '<div><center><a href=&quot;https://www.rottentomatoes.com/m/the_lord_of_the_rings_the_war_of_the_rohirrim&quot; title=&quot;반지의 제왕: 로히림의 전쟁 해외 평점은?&quot;><button class=&quot;btn-hover color-8&quot;>반지의 제왕: 로히림의 전쟁해외 평점 보러 가기 👉</button></a></center></div>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>영화 반지의 제왕: 로히림의 전쟁 </b></p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>해외 평론가들의 평점이 궁금하시다면??</b></p>'
    },
    {
      type: 'body',
      html: '<div><center><a href=&quot;https://search.naver.com/search.naver?sm=tab_hty.top&amp;where=nexearch&amp;ssc=tab.nx.all&amp;query=반지의+제왕+로히림의+전쟁+예매&amp;oquery=서바이브+정보&amp;tqi=iHth5lpzL8wssTfAvudssssssMl-116987&quot; title=&quot;반지의 제왕: 로히림의 전쟁 보러가기&quot;><button class=&quot;btn-hover color-8&quot;>반지의 제왕: 로히림의 전쟁 바로 보러 가기👉</button></a></center></div>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>위 링크를 통해</b></p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>반지의 제왕: 로히림의 전쟁을</b></p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>극장에서 예매해 보세요.</b></p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '반지의 제왕: 로히림의 전쟁 해외 평론가 후기',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>이어서 영화 반지의 제왕: 로히림의 전쟁의 해외 평론가 후기입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post99_img4.jpg', alt: '반지의 제왕: 로히림의 전쟁', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>긍정적인 평가와 부정적인 평가, 그리고 전체적으로 어떠한 평가가 주를 이루고 있는지 함께 확인해 보시길 바랍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>내용에 스포는 크게 없기에 바로 이어서 확인해 보시길 바랍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>작품이 가지는 <b>애니메이션의 시각적 매력과 캐릭터 중심의 서사</b>를 높이 평가하면서도, 일부에서는 이야기의 깊이가 다소 부족하다는 의견을 보이고 있습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '반지의 제왕: 로히림의 전쟁 긍정적인 평가',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p>Sarah Gopaul (Digital Journal)은 &quot;영화 제작자들이 텅 빈 캔버스를 활용해 톨킨의 세계에 등장하는 여성 캐릭터들에게 깊이와 개성을 부여했다&quot;라고 평하며, 특히 헤라(Hèra)와 그녀의 시녀 올윈(Olwyn)이 <b>기존의 전형적인 묘사보다 더 세밀하게 그려졌다</b>고 언급합니다.</p>'
    },
    { type: 'image', src: '/images/post99_img5.jpg', alt: '반지의 제왕: 로히림의 전쟁', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>Juanma Fernández París (El Nuevo Día)는 &quot;이야기의 구성이 탄탄하며, 애니메이션의 예술적 불일치가 영화의 즐거움을 방해하지 않는다&quot;라고 말하며 <b>팬들에게 충분히 매력적인 작품임을 강조</b>했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>Robbie Collin (Daily Telegraph)은 &quot;이 작품이 기존 톨킨의 상징들에 의존하기보다는 독립적인 판타지 애니메이션으로서의 길을 선택했다&quot;라고 평가하며, <b>독창적인 시도를 긍정적</b>으로 보았습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '반지의 제왕: 로히림의 전쟁 부정적인 평가',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p>반면, Charles Solomon (FilmWeek)은 &quot;<b>올해 가장 실망스러운 영화</b>&quot;라고 강하게 비판하며, 스토리의 무게감 부족을 지적했습니다.</p>'
    },
    { type: 'image', src: '/images/post99_img6.jpg', alt: '반지의 제왕: 로히림의 전쟁', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>Ayaan Paul Chowdhury (The Hindu)는 &quot;애니메이션 특유의 초현실주의와 미야자키의 내적 경이로움을 보여주는 순간도 있지만, <b>대체로 안전한 선택에 그친다&quot;라고</b> 아쉬움을 표현했습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section7',
      text: '반지의 제왕: 로히림의 전쟁 전반적인 평가',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p>많은 비평가들이 <b>\'애니메이션이라는 형식을 통한 판타지 장르의 확장성\'</b>을 높게 평가했으며, 특히 헤라를 중심으로 한 여성 서사가 신선하다는 의견이 다수였습니다. 하지만 스토리의 깊이나 감정적인 여운이 부족하다는 비판도 일부 제기되었습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>작품은 <b>기존 반지의 제왕 팬들과 애니메이션 애호가들에게 흥미로운 경험을 제공</b>하지만, <b>스토리 면에서는 기대에 미치지 못한다는 평</b>이 공존합니다.</p>'
    },
    {
      type: 'ending',
      html: '<div><center><a href=&quot;https://search.naver.com/search.naver?sm=tab_hty.top&amp;where=nexearch&amp;ssc=tab.nx.all&amp;query=반지의+제왕+로히림의+전쟁+예매&amp;oquery=서바이브+정보&amp;tqi=iHth5lpzL8wssTfAvudssssssMl-116987&quot; title=&quot;반지의 제왕: 로히림의 전쟁 보러가기&quot;><button class=&quot;btn-hover color-8&quot;>반지의 제왕: 로히림의 전쟁 바로 보러 가기👉</button></a></center></div>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
