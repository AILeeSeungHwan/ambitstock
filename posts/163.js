const post = {
  id: 163,
  sections: [
    {
      type: 'intro',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>영화 1승 시사회 관람평 줄거리 포스팅입니다.</b></span></p><p>2024년 12월 04일 개봉예정인 영화 1승에 대한 모든 정보를 개봉 전 확인해 보시길 바랍니다.</p>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post163_img1.jpg', alt: '영화 1승', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/horror-movie-recommend-guide/',
      text: '2024년 공포 영화 추천 인기 순위 총정리 현재상영작'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '🎥 1승 줄거리',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p>영화 <i>1승</i>은 <b><span style=&quot;background-color: #9feec3;&quot;>인생에서도 &quot;패배 그랜드슬램&quot;을 달성한 배구선수 출신 감독</span> <i>우진</i> (송강호)이 주인공</b>입니다.</p>'
    },
    { type: 'image', src: '/images/post163_img2.jpg', alt: '영화 1승', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>그는 <b>해체 직전의 여자배구팀 <i>핑크스톰</i></b>을 맡게 됩니다.</p>'
    },
    {
      type: 'body',
      html: '<p>에이스 선수의 이탈로 팀은 사실상 최약체로 전락했고,</p>'
    },
    {
      type: 'body',
      html: '<p>구단주는 단 한 번이라도 승리한다면 상금 20억 원을 약속하며 팀에 새로운 목표를 부여합니다.</p>'
    },
    {
      type: 'body',
      html: '<p><br>이 영화는 팀워크가 무너진 선수들이 갈등을 극복하고,</br></p>'
    },
    {
      type: 'body',
      html: '<p>우진 감독이 과거 실패를 딛고 성장하며 팀에 활력을 불어넣는 과정을 그립니다.</p>'
    },
    {
      type: 'body',
      html: '<p>예상 가능한 스포츠 영화의 전형적 전개를 따르며, 마지막에는 &quot;1승&quot;을 이루기 위한 극적인 순간을 보여줍니다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '🎬 영화 1승 시사회 관람평 후기',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<h4><b>1승 시사회 일정</b> </h4>'
    },
    { type: 'image', src: '/images/post163_img3.jpg', alt: '영화 1승', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>시사회는 11월 30일과 12월 1일 양일간 서울 주요 극장에서 열렸습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>배우 송강호와 박정민, 감독 신연식이 참석하며 영화에 대한 팬들과 소통하는 시간을 가졌습니다​.</p>'
    },
    {
      type: 'body',
      html: '<h4><br><b>영화 1승 관람평</b></br></h4>'
    },
    {
      type: 'body',
      html: '<p>영화는 <span style=&quot;background-color: #9feec3;&quot;><b>빠른 전개와 생동감 있는 배구 경기 장면으로 긍정적인 평가</b></span>를 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>롱테이크로 촬영된 경기 장면과 음향 효과가 특히 극찬</b>받았으며,</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>한 평론가는</span></b></span><br/><span style=&quot;color: #781b33;&quot;><b>&quot;여자 배구판 <i>슬램덩크</i>&quot;로 비유</b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>친숙한 플롯을 가진 따뜻한 성장 영화로 소개했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>그러나 <b>예상 가능한 전개와 지나치게 익숙한 스포츠 영화의 전형성 때문에 다소 평범</b>하게 느껴졌다는 의견도 있었습니다​.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '📜 영화 1승의 메시지',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b><i>1승</i>은 단순한 경기 승리 이상의 메시지</b></span>를 담고 있습니다.</p>'
    },
    { type: 'image', src: '/images/post163_img4.jpg', alt: '영화 1승', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>팀워크와 실패를 극복하는 과정을 통해 인간적 성장을 그리고, 개인의 노력과 팀의 조화가 얼마나 중요한지를 보여줍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>결과적으로 승패를 떠나 함께 이뤄낸 변화가 더 값지다는 점을 강조합니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '🎯 영화 1승기대 포인트',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p>영화 1승은 경쾌한 스토리와 배우들의 열연, <span style=&quot;background-color: #9feec3;&quot;><b>박진감 넘치는 경기 장면을 통해 관객들에게 가슴 벅찬 감동을 선사할 예정</b></span>입니다.</p>'
    },
    { type: 'image', src: '/images/post163_img6.jpg', alt: '영화 1승', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post163_img5.jpg', alt: '영화 1승', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>아마 우리가 알고 있는 한국 영화의 맛을 그대로 재연할 것으로 예상 되지만,</p>'
    },
    {
      type: 'body',
      html: '<p>스포츠 영화 팬들에게는 물론 따뜻한 성장 이야기를 찾는 관객들에게도 추천할 만한 작품입니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>12월 04일 개봉 예정이니, 극장에서 직접 확인해보세요!</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' }
  ]
}

module.exports = post
