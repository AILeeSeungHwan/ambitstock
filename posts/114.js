const post = {
  id: 114,
  sections: [
    {
      type: 'intro',
      html: '<p>사일런트 러브 뜻 관람평 결말 후기 포토 <말할 수 없는 소년과 시력을 잃은 소녀></p>'
    },
    {
      type: 'image',
      src: '/images/post114_thumb.svg',
      alt: '사일런트 러브 뜻 관람평 결말 후기 포토 <말할 수 없는 소년과 시력을 잃은 소녀>',
      caption: '사일런트 러브 뜻 관람평 결말 후기 포토 <말할 수 없는 소년과 시력을 잃은 소녀>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '영화 사일런트 러브 개봉일',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>일본 영화 사일런트 러브가 2025년 1월 08일 개봉합니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p>이번 포스팅에서는 <span style=&quot;background-color: #9feec3;&quot;><b>영화 제목인 사일런트 러브의 뜻 해석, 그리고 해외 시청자들의 반응 후기</b></span>를 함께 알아보도록 하겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EB%B3%BC%EB%A7%8C%ED%95%9C-%EC%98%81%ED%99%94-%EB%93%9C%EB%9D%BC%EB%A7%88-%EC%B6%94%EC%B2%9C-%EB%84%B7%ED%94%8C%EB%A6%AD%EC%8A%A4-%EC%8B%9C%EC%B2%AD%EA%B0%80%EB%8A%A5-%EC%9D%B4%EB%8F%99%EC%A7%84-%EB%B0%95%ED%8F%89%EC%8B%9D-%ED%95%9C%EC%A4%84%ED%8F%89-%EC%B6%94%EC%B2%9C%EC%9E%91%ED%92%88',
      text: '볼만한 영화 드라마 추천 넷플릭스 시청가능 <이동진 박평식 한줄평 추천작품>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%9D%B4%EB%8F%99%EC%A7%84-%ED%8F%89%EB%A1%A0%EA%B0%80%EA%B0%80-%EC%B6%94%EC%B2%9C%ED%95%98%EB%8A%94-%EB%B3%84-%EB%8B%A4%EC%84%AF%EA%B0%9C-%EB%AA%85%EC%9E%91-%EB%B3%BC%EB%A7%8C%ED%95%9C-%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-TOP-10',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%EB%B0%95%ED%8F%89%EC%8B%9D-%ED%8F%89%EB%A1%A0%EA%B0%80-%EB%AA%85%EC%9E%91-10%EC%84%A0',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '사일런트 러브 줄거리',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>“그가 어떤 사람이든 상관없어, <br>그 손으로 나를 구해줬어.” </br></span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>방황하던 시절, 한순간의 실수로 목소리를 잃은 ‘아오이’는 학교 청소부로 일하며 무미건조한 삶을 이어간다.</p>'
    },
    {
      type: 'body',
      html: '<p>한때 촉망받던 피아니스트였지만, 교통사고로 시력을 잃은 ‘미카’는 절망 속에서 대학 옥상에서 극단적인 선택을 시도한다.</p>'
    },
    {
      type: 'body',
      html: '<p>“네가 꿈을 이뤘으면 좋겠어. 나한테는 그게 없으니까.”</p>'
    },
    {
      type: 'body',
      html: '<p>마침 그곳에서 작업을 하던 ‘아오이’가 ‘미카’를 말리게 되고, 흐느끼는 ‘미카’를 보며 아오이는 눈을 뗄 수 없다.</p>'
    },
    {
      type: 'body',
      html: '<p>며칠 후, 아오이는 폐강당에서 피아노를 치고 있는 미카를 발견하고, 그녀가 다시 빛을 찾을 수 있도록 자신의 모든 것을 걸고 지키기로 결심한다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '사일런트 러브 영어 제목 뜻',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>사일런트 러브<br/></span>Silent Love</b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p><b>사일런트 러브(Silent Love)</b>의 제목을 단어별로 해석하면 다음과 같습니다:</p>'
    },
    {
      type: 'body',
      html: '<p><b>1. 사일런트(Silent):</b> &quot;사일런트&quot;는 &quot;조용한&quot;, &quot;소리 없는&quot;, &quot;침묵의&quot;라는 의미입니다. 감정이나 표현이 외부로 드러나지 않거나, 말을 하지 않고도 존재하는 어떤 상태를 암시합니다. 이 경우, 아오이와 미카가 서로의 감정을 직설적으로 표현하기보다는 침묵 속에서 감정의 교류가 이루어진다는 의미로 해석될 수 있습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>2. 러브(Love):</b> &quot;러브&quot;는 &quot;사랑&quot;을 의미합니다. 두 사람 사이의 감정적 연결을 나타내며, 서로의 마음과 애정을 바탕으로 한 깊은 관계를 뜻합니다. 이 사랑은 감정적으로 복잡하지만, 외부로 드러나지 않는 \'사일런트\'한 형태로 존재합니다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;font-family: \'Noto Serif KR\'; color: #781b33;&quot;><b> &quot;소리 없이 흐르는 사랑&quot;</b><br/><b>&quot;말하지 않아도 전해지는 사랑&quot;</b> </span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>따라서 <b>&quot;사일런트 러브(Silent Love)&quot;</b>는 &quot;소리 없이 흐르는 사랑&quot;이나 &quot;말하지 않아도 전해지는 사랑&quot;이라는 의미로 해석할 수 있습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '사일런트 러브 해외 시청자 평점',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: #333333; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>사일런트 러브 해외 시청자 평점입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>총 10점 만점의 6.3점으로 높지 않은 평점을 기록하고 있습니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 해외 시청자들의 좋지 못한 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '사일런트 러브 해외 시청자 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>사일런트 러브 해외 시청자 결말후기 관람평입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><b>katsu_miya - 9/10</b><br/>말할 수 없는 소년과 시력을 잃은 소녀의 감동적인 사랑 이야기!</p>'
    },
    {
      type: 'body',
      html: '<p>아름다운 사랑을 다룬 이 영화는 감동적인 로맨스를 그린 작품이다.</p>'
    },
    {
      type: 'body',
      html: '<p>멜랑콜리한 눈빛을 가진 야마다 료스케의 연기는 그의 캐릭터의 마음을 말해준다.</p>'
    },
    {
      type: 'body',
      html: '<p>바로 그 눈빛이 이 영화를 반복해서 보게 만든 이유다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>katsu_miya - 평점 없음</b><br/>주인공의 사랑은 다른 사람에 대한 친절에서 시작된다.</p>'
    },
    {
      type: 'body',
      html: '<p>우치다 에이이치의 캐릭터 중심의 치밀하게 짜여진 로맨스는 순수한 사랑이란 사랑하는 사람을 위해 모든 것을 하고, 사랑이 돌아오지 않는다고 생각해도 자기를 희생할 준비가 되어 있는 것임을 보여준다.</p>'
    },
    {
      type: 'body',
      html: '<p>진정한 사랑은 사회적 계층 간의 차이를 어떻게 메울 수 있는지 전혀 모르며, 중요한 것은 사람의 마음속에 있는 것,</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>즉 값비싼 차,부유한 가족, <br/>피아노 실력 등이 아닌 <br/>사람의 가치를 쌓는 것이라는 점을 <br/>강조한다. </span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>미카가 어떻게 아오이를 사랑하지 않을 수 있을까?</p>'
    },
    {
      type: 'body',
      html: '<p>음악적으로는 피아노뿐만 아니라 하프 음악도 기분 좋고 분위기 있는 소리로, 오케스트라 음악은 훌륭하다.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 사일런트 러브 해외 시청자 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
