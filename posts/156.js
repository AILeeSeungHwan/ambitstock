const post = {
  id: 156,
  sections: [
    {
      type: 'intro',
      html: '<p>하얼빈 시사회 관람평 리뷰 포토 < 토론토 국제 영화제 > 해외 평론가 반응 결말 후기 개봉예정작</p>'
    },
    {
      type: 'image',
      src: '/images/post156_thumb.svg',
      alt: '하얼빈 시사회 관람평 리뷰 포토 < 토론토 국제 영화제 > 해외 평론가 반응 결말 후기 개봉예정작',
      caption: '하얼빈 시사회 관람평 리뷰 포토 < 토론토 국제 영화제 > 해외 평론가 반응 결말 후기 개봉예정작'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '영화 하얼빈 시사회 해외 반응 관람평가',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p>영화 <b>하얼빈</b>은 <span style=&quot;background-color: #9feec3;&quot;><b>토론토국제영화제(TIFF)에서 월드 프리미어</b></span>로 처음 공개되었으며,</p>'
    },
    {
      type: 'body',
      html: '<p>이 시사회는 현지 언론과 관객들에게 큰 관심을 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>시사회의 반응은 다음과 같이 <b>긍정적 관람평과 부정적 관람평</b>으로 나뉘었습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EB%84%B7%ED%94%8C%EB%A6%AD%EC%8A%A4-%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-2024%EB%85%84-9%EC%9B%94-%EC%9D%B8%EA%B8%B0-%EC%9E%91%ED%92%88-%EC%88%9C%EC%9C%84-TOP-10-%EC%B4%9D%EC%A0%95%EB%A6%AC',
      text: '넷플릭스 영화 추천 2024년 9월 인기 작품 순위 TOP 10 총정리!'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%ED%81%AC%EB%A6%AC%EC%8A%A4%ED%86%A0%ED%8D%BC-%EB%86%80%EB%9E%80-%EB%AA%A8%EB%93%A0-%EC%9E%91%ED%92%88-%EB%A1%9C%ED%8A%BC-%ED%86%A0%EB%A7%88%ED%86%A0-%ED%95%B4%EC%99%B8-%ED%8F%89%EB%A1%A0%EA%B0%80-%ED%8F%89%EC%A0%90-%EC%88%9C%EC%9C%84-%EC%88%9C%EC%84%9C%EB%8C%80%EB%A1%9C-%EC%B4%9D%EC%A0%95%EB%A6%AC',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%9D%B4%EB%8F%99%EC%A7%84-%ED%8F%89%EB%A1%A0%EA%B0%80%EA%B0%80-%EC%B6%94%EC%B2%9C%ED%95%98%EB%8A%94-%EB%B3%84-%EB%8B%A4%EC%84%AF%EA%B0%9C-%EB%AA%85%EC%9E%91-%EB%B3%BC%EB%A7%8C%ED%95%9C-%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-TOP-10',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/2024%EB%85%84-%EA%B3%B5%ED%8F%AC-%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%EC%9D%B8%EA%B8%B0-%EC%88%9C%EC%9C%84-%EC%B4%9D%EC%A0%95%EB%A6%AC-%ED%98%84%EC%9E%AC%EC%83%81%EC%98%81%EC%9E%91',
      text: '2024년 공포 영화 추천 인기 순위 총정리 현재상영작'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;https://ambitstock.com/entry/%EC%98%81%ED%99%94-%ED%95%98%EC%96%BC%EB%B9%88-%EC%A0%95%EB%B3%B4-%EC%B6%9C%EC%97%B0%EC%A7%84-%EC%A4%84%EA%B1%B0%EB%A6%AC-%EA%B2%B0%EB%A7%90-%ED%9B%84%EA%B8%B0-2024%EB%85%84-12%EC%9B%94-%EA%B8%B0%EB%8C%80%EB%90%98%EB%8A%94-%ED%95%9C%EA%B5%AD-%EB%8F%85%EB%A6%BD%EC%9A%B4%EB%8F%99-%EA%B0%9C%EB%B4%89%EC%98%88%EC%A0%95%EC%9E%91&quot; rel=&quot;noopener&quot; title=&quot;하얼빈 정보 줄거리 결말 후기&quot;>2024.12.04 - [영화/영화 정보] - 영화 하얼빈 정보 출연진 줄거리 결말 후기 &lt; 2024년 12월 기대되는 한국 독립운동 개봉예정작 &gt;</a></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%ED%95%98%EC%96%BC%EB%B9%88-%EC%A0%95%EB%B3%B4-%EC%B6%9C%EC%97%B0%EC%A7%84-%EC%A4%84%EA%B1%B0%EB%A6%AC-%EA%B2%B0%EB%A7%90-%ED%9B%84%EA%B8%B0-2024%EB%85%84-12%EC%9B%94-%EA%B8%B0%EB%8C%80%EB%90%98%EB%8A%94-%ED%95%9C%EA%B5%AD-%EB%8F%85%EB%A6%BD%EC%9A%B4%EB%8F%99-%EA%B0%9C%EB%B4%89%EC%98%88%EC%A0%95%EC%9E%91',
      text: '영화 하얼빈 정보 출연진 줄거리 결말 후기 < 2024년 12월 기대되는 한국 독립운동 개봉예정작 >'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '하얼빈 시사회 긍정적인 관람평',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<h4><b>1. 긴장감 넘치는 연출 - 역시 우민호!</b></h4>'
    },
    {
      type: 'body',
      html: '<p>우민호 감독의 <b>치밀한 연출과 몰입감 높은 전개</b>가 관객들로부터 찬사를 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>2. 믿고 보는 현빈과 이동욱, 배우들의 열연</b></h4>'
    },
    {
      type: 'body',
      html: '<p>특히 <b>현빈의 감정 연기와 이동욱의 신선한 역할 해석</b>이 관객들의 주목을 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>3. 일제강점기와 독립운동 역사적 메시지</b></h4>'
    },
    {
      type: 'body',
      html: '<p>일제강점기와 독립운동이라는 주제를 강렬하게 전달하며 역사적 메시지가 깊은 인상을 남겼다는 평가가 있었습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이미 여러 영화를 통해 <span style=&quot;background-color: #9feec3;&quot;><b>해외에서도 조선, 한국의 일제강점기와 독립운동에 대한 역사를 이해</b></span>하고 있기 때문에 <b>시대적 배경을 접근하는 것에 있어서도 어려움이 없었다는 평가</b>가 지배적입니다.</p>'
    },
    {
      type: 'body',
      html: '<h4>4. 미장센과 촬영 기법</h4>'
    },
    {
      type: 'body',
      html: '<p>세부적인 장면 구성과 아름다운 촬영 기법이 돋보였다는 반응도 있었습니다.</p>'
    },
    {
      type: 'body',
      html: '<div><center><a href=&quot;https://ambitstock.com/346&quot; title=&quot;하얼빈 출연진 줄거리 결말 보러가기&quot;><button class=&quot;primary&quot;>하얼빈 출연진 줄거리 결말 보러가기</button></a></center></div>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '하얼빈 시사회 부정적 관람평 리뷰',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<h4><b>1. 다소 느린 템포</b></h4>'
    },
    {
      type: 'body',
      html: '<p>일부 관객들은 <b>중간 부분이 조금 늘어진다는 느낌</b>을 받았다는 의견을 제시했습니다.</p>'
    },
    {
      type: 'body',
      html: '<h4><br><b>2. 역사적 디테일의 아쉬움</b></br></h4>'
    },
    {
      type: 'body',
      html: '<p><b>역시나 우리 대한민국의 역사</b>이기에, <b>해외 시사회 관객들에게는 여전히 <span style=&quot;background-color: #9feec3;&quot;>역사적 배경 설명이 부족하다는 평</span></b>이 있었습니다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>3. 다소 익숙한 이야기 전개</b></h4>'
    },
    {
      type: 'body',
      html: '<p>기존 <span style=&quot;background-color: #9feec3;&quot;><b>독립운동 영화와의 유사성 때문에 신선함이 부족하다는 의견</b></span>도 나왔습니다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>4. 캐릭터 개발 부족</b></h4>'
    },
    {
      type: 'body',
      html: '<p><b>일부 캐릭터의 매력과 서사가 충분하지 못하다는 지적</b>도 있었습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '영화 하얼빈 토론토국제영화제 종합 평가',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p>토론토국제영화제에서 <b>하얼빈의 평가는 부정적인 평가보다는 긍정적인 평가가 지배적이었습니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p>이번 겨울 극장가에서 큰 주목을 받는 작품으로 자리 잡을 가능성을 보여주었으며, 실제로 12월 가장 기대되는 작품 중 하나입니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>영화 하얼빈은 12월 25일 크리스마스에 영화관에서 관람하실 수 있습니다. </p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
