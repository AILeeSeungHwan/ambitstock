const post = {
  id: 169,
  sections: [
    {
      type: 'intro',
      html: '<p>나우 이즈 굿 관람평 결말후기 리뷰 다코타 패닝 영화 재개봉 <해외 반응 평점></p>'
    },
    {
      type: 'image',
      src: '/images/post169_thumb.svg',
      alt: '나우 이즈 굿 관람평 결말후기 리뷰 다코타 패닝 영화 재개봉 <해외 반응 평점>',
      caption: '나우 이즈 굿 관람평 결말후기 리뷰 다코타 패닝 영화 재개봉 <해외 반응 평점>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post169_img1.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/netflix-movie-recommend-guide-top-10/',
      text: '넷플릭스 영화 추천 2024년 9월 인기 작품 순위 TOP 10 총정리!'
    },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '2024년 12월 11일 재개봉 나우 이즈 굿 관람평 후기',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '나우 이즈 굿 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>나우 이즈 굿 로튼 토마토 평론가 평점입니다.</b></span><span style=&quot;background-color: #f3c000;&quot;></span></p>'
    },
    { type: 'image', src: '/images/post169_img2.jpg', alt: '나우 이즈 굿 로튼 토마토 평점', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><figure class=&quot;imageblock widthContent&quot; data-origin-height=&quot;588&quot; data-origin-width=&quot;1890&quot;><span data-alt=&quot;나우 이즈 굿 로튼 토마토 평점&quot; data-phocus=&quot;https://blog.kakaocdn.net/dna/bwtWRS/btsK0l6rN50/AAAAAAAAAAAAAAAAAAAAAH83SwGUNV-1vsnLOzTzfrBIombgMcPQSBnxqEiOZrnI/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=%2BsgyMQTTKGxmXx2hjcVotBT%2BPFY%3D&quot; data-url=&quot;https://blog.kakaocdn.net/dna/bwtWRS/btsK0l6rN50/AAAAAAAAAAAAAAAAAAAAAH83SwGUNV-1vsnLOzTzfrBIombgMcPQSBnxqEiOZrnI/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=%2BsgyMQTTKGxmXx2hjcVotBT%2BPFY%3D&quot;></span><figcaption>나우 이즈 굿 로튼 토마토 평점</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 56%, 관객 평점은 64%를 기록하였습니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>높지 않은 평점과 더불어 썩은 토마토 마크를 받으며 평론가들로부터 부정적 평가를 받은 나우 이즈 굿</p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분의 평론가들의 나쁜 평점을 이끌었는지 결말 후기 관람평 확인해 보겠습니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '나우 이즈 굿 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>나우 이즈 굿 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post169_img3.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><b>Joseph Walsh - 평점 1/5</b><br>나우 이즈 굿 ‘Now Is Good’는 피해야 할 영화이다.</br></p>'
    },
    {
      type: 'body',
      html: '<p>사실 이 영화는 좋지 않으며, 대신 피곤하고, 종종 불쾌한 드라마로, 정직함이나 섬세함이 거의 없거나 전혀 없다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Tracy Moore - 평점 4/5</b><br/>십 대 암 투병을 다룬 감동적인 눈물샘을 자극하는 영화로, 성숙한 주제를 다룬다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Brian Orndorf - 평점 D+</b><br/>주인공의 매력이 부족하고 예측 가능한 전개를 가진 영화로, 젊은 성인 문학의 기원을 따르려 했지만 그에 어울리는 음악이 전혀 없다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Andy Lea - 평점 3/5</b><br/>말기 질병을 다룬 로맨틱 드라마에서 얻을 수 있는 감정적 울림이 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Amber Wilkinson - 평점 2.5/5</b><br/>올 파커의 최신 감독 작품은 암이라는 외피를 가지고 있지만, 그 중심에 있는 소원 성취적인 십 대 로맨스가 주요한 관객층에게 어필할 것이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mary Clare Waireri - 평점 2/5</b><br/>나우 이즈 굿 ‘Now Is Good’의 문제는, 이 영화가 십대 관객을 울게 만든다는 점보다는, 가장 뻔하고 감상적인 버튼을 반복적으로, 끊임없이, 과도하게 눌러댄다는 점이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Lisa Giles-Keddie - 평점 3/5</b><br/>다코타 패닝은 이 작품에서 정말 인상 깊고 기억에 남는 연기를 보여준다. 이는 파커 감독에게 또 하나의 숨겨진 경쟁력을 선사할 것이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Rich Cline - 평점 2.5/5</b><br/>사춘기 소녀들은 이 감상적인 로맨스를 너무나 로맨틱하게 느낄 수 있지만, 그 외의 관객들은 영화를 끝까지 보기 힘들 것이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Donald Clarke - 평점 4/5</b><br/>나우 이즈 굿 ‘Now Is Good’가 대단한 영화인가? 아마도 그렇지 않다.</p>'
    },
    {
      type: 'body',
      html: '<p>하지만 그 감정적 효과는 매우 강렬하다.</p>'
    },
    {
      type: 'body',
      html: '<p>온통 휴지 더미를 가져가라.</p>'
    },
    {
      type: 'body',
      html: '<p><b>MaryAnn Johanson - 평점 없음</b><br/>왜 [다코타 패닝]은 이렇게 여성 주인공이 좀처럼 누리지 못하는 삶을 살고 있는가? 그녀는 죽어가고 있기 때문이다. *한숨*</p>'
    },
    {
      type: 'body',
      html: '<p><b>Brian Henry Martin - 평점 3/10</b><br/>다코타 패닝의 강력한 죽어가는 연기에도 불구하고, <span style=&quot;color: #777777; text-align: center;&quot;>나우 이즈 굿 </span>‘Now Is Good’은 좋은 영화가 아니다. 안타깝게도 90년대의 낮 시간대 소프 드라마처럼 보이고, 마이클 볼튼의 머리카락처럼 감성적인 자극을 준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Derek Malcolm - 평점 2/5</b><br/>이 영화는 너무나 뻔한 감정적 버튼을 지나치게 강하게 눌러서, 완전히 조종당하는 느낌을 준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Allan Hunter - 평점 3/5</b><br/>휴지 한 상자를 준비하라.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Jason Best - 평점 4/5</b><br/>십대 소녀들은 마치 물이 쏟아지는 소화전처럼 감정적으로 반응할 것이다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '나우 이즈 굿 Now Is Good 뜻 의미 해석',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b>Now Is Good</b></span><br/><span style=&quot;color: #781b33;&quot;><b>지금이 좋다</b></span></blockquote>'
    },
    { type: 'image', src: '/images/post169_img9.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post169_img8.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post169_img7.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post169_img6.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post169_img5.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post169_img4.jpg', alt: '나우 이즈 굿', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><b>나우 이즈 굿의 뜻은 지금이 좋다입니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p>주인공 다코타 패딩이 백혈병 진단을 받은 후 죽음을 앞둔 상황에서도 하루하루 충실히 살아가는 그 모습을 보여줍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>즉, 제한적인 시간을 가진 주인공이 Now, 지금에 충실하며 살아가는 태도를 보여주는 제목입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>그리고 제한된 시간 속에서도 현재를 긍정적으로 바라보며 삶을 바라보는 관점에 대해 다시 한번 생각해 볼 수 있는 영화입니다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>현재에 충실하라<br/>지금이 좋다</span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 나우 이즈 굿 로튼 토마토 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
