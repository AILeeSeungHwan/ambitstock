const post = {
  id: 78,
  sections: [
    {
      type: 'intro',
      html: '<p>2025년 9월 개봉한 영화 <b>빌리브(Believe)</b>는 제목 그대로 ‘믿음’이라는 화두를 던지는 작품입니다.</p><p>세 명의 감독이 각각의 시선으로 하나의 질문을 풀어내며, 짧지만 강렬한 옴니버스 형식으로 구성되었습니다. 믿음이란 단어는 단순히 긍정이나 희망을 상징하는 것이 아니라, 현실 속에서 흔들리고 시험받는 감정을 그대로 담아내는 키워드입니다. 이 작품은 현실과 환상, 의심과 확신, 부정과 회복을 넘나들며 관객에게 “당신은 무엇을 믿고 있습니까?”라는 질문을 던집니다.</p><p>짧은 러닝타임 속에서도 각각의 단편은 뚜렷한 개성과 메시지를 지니고 있습니다. 형사물의 긴장감, 멜로의 애잔함, 스포츠 드라마의 열정이 어우러져 다양한 관점에서 ‘믿음’을 바라볼 수 있도록 만들어졌습니다. 지금부터 영화 <i>빌리브</i>의 주요 정보와 줄거리, 출연진, 그리고 제목의 깊은 의미까지 차례대로 살펴보겠습니다.</p>'
    },
    {
      type: 'image',
      src: '/images/post78_thumb.svg',
      alt: '빌리브 뜻 정보 후기 | 세명의 감독이 풀어내는 요즘 대세 옴니버스 영화!',
      caption: '빌리브 뜻 정보 후기 | 세명의 감독이 풀어내는 요즘 대세 옴니버스 영화!'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '영화 빌리브 정보',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<ul style=&quot;list-style-type: disc;&quot;> <li><b>개봉일:</b> 2025년 9월 17일</li> <li><b>국가:</b> 한국</li> <li><b>장르:</b> 드라마 / 스릴러 / 액션 / 로맨스 / 멜로</li> <li><b>러닝타임:</b> 37분</li> <li><b>감독:</b> 이종석, 라희찬, 박범수</li> <li><b>등급:</b> 12세 이상 관람가</li> </ul>'
    },
    { type: 'image', src: '/images/post78_img1.webp', alt: '영화 빌리브', caption: 'ⓒ R의 필름공장' },
    {
      type: 'h2',
      id: 'section2',
      text: '줄거리',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><i>빌리브</i>는 세 가지 단편으로 구성된 옴니버스 영화입니다.</p>'
    },
    { type: 'image', src: '/images/post78_img2.jpg', alt: '빌리브', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>이종석 감독의 단편에서는 빨간 원피스를 입은 여자를 쫓는 형사 ‘태수’가 등장합니다. 현실과 환상이 뒤섞이며, 그가 붙잡으려는 대상은 실체 없는 ‘믿음’ 그 자체일 수도 있다는 긴장감을 자아냅니다.</p>'
    },
    {
      type: 'body',
      html: '<p>라희찬 감독의 단편은 ‘창석’의 이야기를 다룹니다. 일상적인 세계를 멜로와 액션, 코미디의 리듬으로 변주하며, 현실의 무게와 상상의 자유가 어떻게 교차하는지를 보여줍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>박범수 감독의 단편은 아이스하키 선수 ‘치우’의 이야기입니다. 늦은 시각 혼자 훈련하며 스스로를 단련하는 과정을 통해, 스포츠라는 땀과 고통 속에서 믿음을 되찾아가는 성장의 순간을 그려냅니다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '출연진',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p>형사 ‘태수’ 역에는 <b>강기영</b>이 출연하여 현실적인 연기와 특유의 인간적인 매력을 보여줍니다.</p>'
    },
    { type: 'image', src: '/images/post78_img3.jpg', alt: '영화 빌리브', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>옴니버스 각각의 단편에는 <b>주진모</b>, <b>박호산</b>, <b>고창석</b>이 합류해 각자의 색깔을 더했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>짧은 러닝타임에도 배우들의 진중한 연기가 돋보이며, 서로 다른 결을 지닌 이야기들을 더욱 단단하게 이어줍니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '비하인드와 감독의 시선',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p>세 명의 감독은 “믿음이라는 단어가 가진 다층적인 의미를 풀어내고 싶었다”는 공통된 화두로 작품을 기획했습니다.</p>'
    },
    { type: 'image', src: '/images/post78_img4.jpg', alt: '영화 빌리브 줄거리', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>짧은 러닝타임 속에서 각 단편의 색깔을 극명히 나누고, 서로 다른 정서가 하나의 주제로 수렴되도록 구성했습니다. 인터뷰에서는 현실과 환상의 경계를 넘나드는 형사 이야기, 유머와 액션을 결합한 창석의 세계, 스포츠를 통한 자기 극복이라는 세 가지 축이 “믿음”이라는 키워드 아래 모이는 과정을 강조했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>관객들이 영화 속에서 ‘믿음’을 다시 정의하고, 각자 다른 방식으로 질문을 던질 수 있기를 바랐다는 점도 언급되었습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '제목 ‘빌리브(Believe)’의 뜻',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p><i>빌리브</i>라는 제목은 신념을 넘어선 의미를 담고 있습니다.</p>'
    },
    { type: 'image', src: '/images/post78_img7.jpg', alt: '영화 빌리브', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post78_img6.jpg', alt: '빌리브', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post78_img5.jpg', alt: '영화 빌리브', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>첫째, 믿음은 언제나 시험받는 감정이라는 점입니다. 형사 태수의 환상은 ‘믿음의 불확실성’을 보여줍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>둘째, 믿음은 관계 속에서 드러난다는 점입니다. 창석의 이야기는 일상에서 작은 믿음을 발견하는 과정을 담고 있습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>셋째, 믿음은 스스로를 되돌아보게 만드는 힘이라는 점입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>치우의 고독한 훈련은 ‘자기 자신을 믿는 것’의 소중함을 전합니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>빌리브</b>라는 제목은 “당신은 무엇을 믿습니까?”라는 질문을 관객에게 던집니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이 영화가 말하는 믿음은 희망이나 신앙이상의, 삶의 매 순간에 부딪히는 선택과 회복의 문제입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>그렇기 때문에 영화 <i>빌리브</i>는 짧지만 강한 울림을 주며, 보는 이로 하여금 스스로의 믿음을 다시 돌아보게 만드는 작품입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>곧 개봉하는 영화 빌리브, 영화관에서 꼭 확인해 보시길 바랍니다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'ending',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/movie-recommend-rating-top-9/',
      text: '영화 추천 쿠엔틴타란티노 필모그래피 수상작품 평점 인기 순위 TOP 9'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
