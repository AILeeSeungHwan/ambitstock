const post = {
  id: 132,
  sections: [
    {
      type: 'intro',
      html: '<p>노스페라투 관람평 포토 결말 후기 뜻 해석 < 1월 공포영화는 무조건 이 작품 ></p>'
    },
    {
      type: 'image',
      src: '/images/post132_thumb.svg',
      alt: '노스페라투 관람평 포토 결말 후기 뜻 해석 < 1월 공포영화는 무조건 이 작품 >',
      caption: '노스페라투 관람평 포토 결말 후기 뜻 해석 < 1월 공포영화는 무조건 이 작품 >'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '노스페라투 관람평 후기 뜻 해석',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '노스페라투 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: #333333; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>노스페라투 로튼 토마토 평론가 평점입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><figure class=&quot;imageblock widthContent&quot; data-origin-height=&quot;572&quot; data-origin-width=&quot;1730&quot;><span data-alt=&quot;노스페라투 로튼 토마토 평론가 평점&quot; data-phocus=&quot;https://blog.kakaocdn.net/dna/bBWDo5/btsLCtOGdmZ/AAAAAAAAAAAAAAAAAAAAAK8clref9WF37S072vSsstzR-qAkMmxvQ8oqU4bahTQf/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=7uMn9CV%2BsTKPNjdDVxR6M2v6N4o%3D&quot; data-url=&quot;https://blog.kakaocdn.net/dna/bBWDo5/btsLCtOGdmZ/AAAAAAAAAAAAAAAAAAAAAK8clref9WF37S072vSsstzR-qAkMmxvQ8oqU4bahTQf/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=7uMn9CV%2BsTKPNjdDVxR6M2v6N4o%3D&quot;></span><figcaption>노스페라투 로튼 토마토 평론가 평점</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 86%, 관객 평점 76%입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>80점대 중반의 높은 평론가 평점과 더불어 <b>로튼 토마토 신선도 보증 마크 또한 획득하며 좋은 평가</b>를 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>동일한 시기에 한국에서 개봉하는 더 엑소시즘과는 완전히 다른 평점을 기록하였습니다. 혹여 2025년 공포영화를 볼 계획이 있으시다면 해당 영화를 선택하시길 바랍니다.</p>'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;https://ambitstock.com/entry/%EB%8D%94-%EC%97%91%EC%86%8C%EC%8B%9C%EC%A6%98The-Exorcism-%EA%B4%80%EB%9E%8C%ED%8F%89-%ED%8F%AC%ED%86%A0-%EA%B2%B0%EB%A7%90-%ED%9B%84%EA%B8%B0%EB%B0%B0%EC%9A%B0%EB%8A%94-%EC%A3%84%EA%B0%80-%EC%97%86%EB%8B%A4&quot; rel=&quot;noopener&quot; title=&quot;더 엑소시즘&quot;>더 엑소시즘(The Exorcism) 관람평 포토 결말 후기 &lt;배우는 죄가 없다&gt;</a></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EB%8D%94-%EC%97%91%EC%86%8C%EC%8B%9C%EC%A6%98The-Exorcism-%EA%B4%80%EB%9E%8C%ED%8F%89-%ED%8F%AC%ED%86%A0-%EA%B2%B0%EB%A7%90-%ED%9B%84%EA%B8%B0%EB%B0%B0%EC%9A%B0%EB%8A%94-%EC%A3%84%EA%B0%80-%EC%97%86%EB%8B%A4',
      text: '더 엑소시즘(The Exorcism) 관람평 포토 결말 후기<배우는 죄가 없다>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 평론가들의 좋은 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '노스페라투 관람평 결말후기 포토',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>노스페라투  관람평 평론가 결말후기 포토입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><b>Richard Propes - 평점 3.5/4.0 (Original Score: 3.5/4.0)</b></p>'
    },
    {
      type: 'body',
      html: '<p>영화는 적절하게 &lt;노스페라투&gt;라는 제목을 가졌지만, 여기서 펼쳐지는 모든 사건의 중심에는 엘렌 역의 조니 뎁이 있다. 그는 올해 가장 영감을 주는 연기 중 하나를 보여준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Joe Friar - 평점 4/4 (Original Score: 4/4)</b><br/>릴리로즈 뎁은 화면을 불태우고, 빌 스카르스고르드는 몰락하는 인간 시체처럼 보이는 트란실바니아의 흡혈귀로서 두려움을 준다. 이 중독적인 영화는 서서히 관객을 유혹하다가 혐오감으로 폭발한다. 2024년 최고의 영화 중 하나이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Chase Hutchinson - 평점 3.5/4 (Original Score: 3.5/4)</b><br/>이 영화는 영화 역사상 첫 번째 흡혈귀 중 하나의 그림자에서 벗어나려 하기보다는, 그 안에서 아름다운 어둠을 발견하고 새로운 경외와 공포의 감각을 불어넣는다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>John Urbancich - 평점 4/5 (Original Score: 4/5)</b><br/>이 영화는 1920년대 클래식의 스타일에 집중하며, 로버트 에거스 감독이 각본과 연출을 맡은 최신 작품의 상당히 눈여겨볼 만한 각색이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Anthony Morris - 평점 없음</b><br/>이 영화에는 공연적인 분위기가 있다. 우리가 익숙한 이야기를 다시 보고 있다는 인식을 가지고 있다. 이 의식적인 분위기가 &lt;노스페라투&gt; 버전의 강점을 만들어낸다. 최고의 순간에서는 이 영화를 보는 것이 마치 불경한 소환식을 목격하는 것처럼 느껴진다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>M.V. Moorhead - 평점 없음</b><br/>영화는 틀림없이 악몽 같은 분위기를 만들어낸다. 대화와 조연들의 연기도 강력하다. 그러나 이 &lt;노스페라투&gt;는 자신을 속이며, 주인공을 멀리 떨어뜨리면서 그 자체로 속임수를 쓰고 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Jocelyn Noveck - 평점 2.5/4 (Original Score: 2.5/4)</b><br/>1922년 무성 흡혈귀 클래식의 업데이트 버전은 당신을 뼛속까지 얼어붙게 할 것이다. 그러나 당신을 겁주지는 않을지도 모른다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Harry Thorfinn-George - 평점 3/5 (Original Score: 3/5)</b><br/>에거스의 &lt;노스페라투&gt;는 예의 바르고 능숙하지만, &quot;공포의 교향곡&quot;(Murnau의 원래 부제)은 아니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Kyle Smith - 평점 없음</b><br/><i>&lt;노스페라투&gt;는 방해받지 않고 영화관에서 보는 것이 가장 좋다. 특히 바람이 울부짖고 비가 퍼붓는 밤에. 이 영화는 1922년 원작의 부제 &quot;공포의 교향곡&quot;에 완전히 부합한다.</i></p>'
    },
    {
      type: 'body',
      html: '<p><b>David Sims - 평점 없음</b><br/>감독의 철저한 연출이 일부 장면을 압도하여 실제 공포의 감각을 방해한다. 에거스는 때때로 지루함에 빠지더라도 여전히 나를 놀라게 한다. 그는 단순히 알 수 없는 것에 대한 두려움을 일으키는 방법을 안다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Kelly Vance - 평점 없음</b><br/>자린 블라쉬케의 음침한 흑백 촬영은 과거의 흡혈귀들을 기리려는 의도를 가진 이 영화에서 계속 보는 이유 중 가장 좋은 점이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Shannon Connellan - 평점 없음</b><br/>전통적인 전설에 대한 창의적인 접근에도 불구하고, &lt;노스페라투&gt;는 어둠에 대한 갈증을 채워주지 못했다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Edwin Arnaudin - 평점 B- (Original Score: B-)</b><br/>괜찮은 각색이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Brent McKnight - 평점 없음</b><br/>많은 점에서 칭찬할 부분이 있지만, &lt;노스페라투&gt;는 큰 반향을 일으키지 못하고, 단지 어깨를 으쓱할 정도의 반응만을 이끌어낸다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '노스페라투(Nosferatu) 뜻 의미 해석',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><b>‘노스페라투’는 루마니아어에서 유래한 단어로, ‘흡혈귀’나 ‘괴물’을 의미합니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p>원래는 루마니아 민속에서 나온 용어로, 죽은 자가 살아 돌아와 사람의 피를 빨아먹는 괴물을 지칭합니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이 단어는 1922년에 제작된 독일의 고전 흡혈귀 영화 *노스페라투(Nosferatu)*에서 더욱 널리 알려졌으며, 이 영화는 당시의 흡혈귀 이야기를 현대적인 감각으로 재구성한 작품으로 유명해졌죠.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 노스페라투 로튼 토마토 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'body',
      html: '<p>감사합니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'ending',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%ED%81%AC%EB%A6%AC%EC%8A%A4%ED%86%A0%ED%8D%BC-%EB%86%80%EB%9E%80-%EB%AA%A8%EB%93%A0-%EC%9E%91%ED%92%88-%EB%A1%9C%ED%8A%BC-%ED%86%A0%EB%A7%88%ED%86%A0-%ED%95%B4%EC%99%B8-%ED%8F%89%EB%A1%A0%EA%B0%80-%ED%8F%89%EC%A0%90-%EC%88%9C%EC%9C%84-%EC%88%9C%EC%84%9C%EB%8C%80%EB%A1%9C-%EC%B4%9D%EC%A0%95%EB%A6%AC',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%9D%B4%EB%8F%99%EC%A7%84-%ED%8F%89%EB%A1%A0%EA%B0%80%EA%B0%80-%EC%B6%94%EC%B2%9C%ED%95%98%EB%8A%94-%EB%B3%84-%EB%8B%A4%EC%84%AF%EA%B0%9C-%EB%AA%85%EC%9E%91-%EB%B3%BC%EB%A7%8C%ED%95%9C-%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-TOP-10',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%EB%B0%95%ED%8F%89%EC%8B%9D-%ED%8F%89%EB%A1%A0%EA%B0%80-%EB%AA%85%EC%9E%91-10%EC%84%A0',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EB%A7%88%EB%B8%94-%EC%98%81%ED%99%94-%EA%B0%9C%EB%B4%89-%EC%98%88%EC%A0%95-%EC%B4%9D-%EC%A0%95%EB%A6%AC-2025%EB%85%84-2027%EB%85%84',
      text: '마블 영화 개봉 예정 총 정리 2025년 - 2027년'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%ED%95%B4%EB%A6%AC%ED%8F%AC%ED%84%B0-%EC%8B%9C%EB%A6%AC%EC%A6%88-%EC%9D%B8%EA%B8%B0-%EC%88%9C%EC%9C%84-%EC%88%9C%EC%84%9C-%EC%B6%94%EC%B2%9C-%ED%8F%89%EC%A0%90-TOP-8',
      text: '해리포터 시리즈 인기 순위 순서 추천 평점 TOP 8'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
