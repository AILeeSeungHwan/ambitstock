const post = {
  id: 331,
  sections: [
    {
      type: 'intro',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>아무도 없는 숲속에서 해외 평점과 후기반응 전해드립니다.</b></span></p><p>한국에서의 평점은 썩 좋지 못해 공개전 기대치에 미치지 못한 작품이 된 아무도 없는 숲속에서,</p><p>해외에서의 인기와 후기 반응은 어떠한지 살펴보시죠.</p>'
    },
    {
      type: 'image',
      src: '/images/post331_thumb.svg',
      alt: '넷플릭스 드라마 아무도 없는 숲속에서 해외 평점과 후기 반응 볼만한 시리즈인가?',
      caption: '넷플릭스 드라마 아무도 없는 숲속에서 해외 평점과 후기 반응 볼만한 시리즈인가?'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'body',
      html: '<p>예상하시겠지만 전체적으로 고민시의 연기력과 분위기, 외모를 칭찬하는 리뷰가 상당히 많습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>IMDB 아무도 없는 숲속에서 평점입니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p><figure><figcaption>아무도 없는 숲속에서 해외평점</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><b>총 표본 546개 7.2점을 기록하였습니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p>국내에서의 평가보다는 높은 점수로, 인기순위도 2,180위로 상승중입니다.</p>'
    },
    {
      type: 'body',
      html: '<p><figure><figcaption>아무도 없는 숲속에서 평점분포</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p>평점 분포는 10점을 준 시청자가 많았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>평점 국가 분포는 인도 미국 인도네시아 영국 한국 순으로 기록되었습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>아무도 없는 숲속에서 후기 반응 전해드립니다. </p>'
    },
    {
      type: 'body',
      html: '<p>리뷰를 요약하여 핵심 내용만 간략하게 의역하였습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>인내는 미덕이다 - Patience Is A Virtue (7/10)</b></p>'
    },
    {
      type: 'body',
      html: '<p>시리즈가 느리게 전개되지만, 지루하지 않고 이야기를 잘 풀어냈다고 평가. 그러나 현재 타임라인과 &quot;펜션 하우스&quot;에서 벌어지는 사건들이 제대로 연결되지 않은 점이 아쉬웠다고 함.</p>'
    },
    {
      type: 'body',
      html: '<p><b>흥미로운 서사와 훌륭한 연기 - Intriguing Narrative &amp; Great Acting! (8/10)</b></p>'
    },
    {
      type: 'body',
      html: '<p>이야기가 흥미롭고 배우들의 연기가 훌륭했다고 평가. 특히 사이코패스 역할을 맡은 배우의 연기를 칭찬하며, 시리즈 후반부의 긴장감이 매우 좋았다고 언급.</p>'
    },
    {
      type: 'body',
      html: '<p><b>미니시리즈로 더 좋았을 텐데 - Would have been great Miniseries (6/10)</b></p>'
    },
    {
      type: 'body',
      html: '<p>시리즈의 기본 아이디어는 좋았지만, 너무 늘어지고 불필요한 장면들이 많아 잠재력을 살리지 못했다고 평가. 주인공이 간단한 해결책을 회피하는 점도 지적됨.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><b>나쁜 평가를 무시하세요 - Ignore the bad reviews (10/10)</b></p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;>초반부의 느린 전개가 필요했다고 옹호하며, 고민시의 연기가 인상적이었다고 평가. 전반적으로 잘 만들어진 시리즈로, 무거운 분위기가 잘 표현되었다고 함.</p>'
    },
    {
      type: 'body',
      html: '<p><b>이상하지만 끝까지 보세요 - Strange but wait! (6/10)</b></p>'
    },
    {
      type: 'body',
      html: '<p>혼란스러운 타임라인과 캐릭터 설정이 이해하기 어렵지만, 주인공의 강렬한 연기가 시리즈를 이끌었다고 평가. 그러나 전반적인 스토리 전개에는 부족함이 있다고 언급.</p>'
    },
    {
      type: 'body',
      html: '<p><b>느리지만 몰입되는 - Slow but engaging (10/10)</b></p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;>고민시의 팬인 리뷰어가 느린 전개에도 불구하고 흥미를 잃지 않았다고 평가. 다만 몇 가지 이해할 수 없는 캐릭터 행동을 지적하며, 전체적으로는 만족스러운 시청 경험이었다고 함.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><b>모든 시청자를 모욕하는 작품! - An insult to every viewer! (1/10)</b></p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;>스토리가 전혀 연결되지 않으며, 불필요한 장면들로 시간을 낭비했다고 강하게 비판. 특히 악당 캐릭터의 말도 안 되는 행동과 사건 전개가 불합리하다고 언급.</p>'
    },
    {
      type: 'body',
      html: '<p><b>이 작품의 이름을 \'달팽이\'로 바꿔야 해 - They should change the name of this show to \'The Snail\' (3/10)</b></p>'
    },
    ,
    { type: 'image', src: '/images/post331_img7.jpg', alt: '아무도 없는 숲속에서', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post331_img6.jpg', alt: '아무도 없는 숲속에서', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post331_img5.jpg', alt: '아무도 없는 숲속에서', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post331_img4.jpg', alt: '아무도 없는 숲속에서', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post331_img3.jpg', alt: '아무도 없는 숲속에서 평점분포', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post331_img2.jpg', alt: '아무도 없는 숲속에서 해외평점', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post331_img1.jpg', alt: '아무도 없는 숲속에서', caption: 'ⓒ R의 필름공장' },
    {
      type: 'ending',
      html: '<p>첫 에피소드가 너무 느리게 전개되며, 새로운 캐릭터가 너무 자주 등장해 집중하기 어렵다고 비판. 이후 에피소드를 볼 동기를 전혀 주지 못했다고 평가.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
