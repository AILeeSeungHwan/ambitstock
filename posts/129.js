const post = {
  id: 129,
  sections: [
    {
      type: 'intro',
      html: '<p>해리 포터와 죽음의 성물 2 재개봉 < 시리즈 최고작품은? 결말 후기 ></p>'
    },
    {
      type: 'image',
      src: '/images/post129_thumb.svg',
      alt: '해리 포터와 죽음의 성물 2 재개봉 < 시리즈 최고작품은? 결말 후기 >',
      caption: '해리 포터와 죽음의 성물 2 재개봉 < 시리즈 최고작품은? 결말 후기 >'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '해리 포터 시리즈 중 단연 1등! 죽음의 성물 2 평론가 평점 후기',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p>해리 포터와 죽음의 성물 2가 2025년 1월 15일 돌아옵니다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;color: #333333; text-align: start;&quot;>해리포터 시리즈 중 당연 최고라고 말할 수 있을 정도의 이 작품,<span> </span></span>이번 포스팅에서는 <b>죽음의 성물 2가 왜 시리즈 최고작품인지</b> 해외 평론가 평점과 후기를 바탕으로 알아보도록 하겠습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>총 8개의 작품의 모든 순위를 아래 포스팅에서 확인 가능합니다.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;https://ambitstock.com/entry/%ED%95%B4%EB%A6%AC%ED%8F%AC%ED%84%B0-%EC%8B%9C%EB%A6%AC%EC%A6%88-%EC%9D%B8%EA%B8%B0-%EC%88%9C%EC%9C%84-%EC%88%9C%EC%84%9C-%EC%B6%94%EC%B2%9C-%ED%8F%89%EC%A0%90-TOP-8&quot; rel=&quot;noopener&quot;>해리포터 시리즈 모든 작품 평점 순위 알아보기</a></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%ED%95%B4%EB%A6%AC%ED%8F%AC%ED%84%B0-%EC%8B%9C%EB%A6%AC%EC%A6%88-%EC%9D%B8%EA%B8%B0-%EC%88%9C%EC%9C%84-%EC%88%9C%EC%84%9C-%EC%B6%94%EC%B2%9C-%ED%8F%89%EC%A0%90-TOP-8',
      text: '해리포터 시리즈 인기 순위 순서 추천 평점 TOP 8'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '해리 포터와 죽음의 성물 2 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: #333333; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>해리 포터와 죽음의 성물 2 로튼 토마토 평론가 평점입니다.</b></span><span style=&quot;background-color: #f3c000;&quot;></span></p>'
    },
    {
      type: 'body',
      html: '<p><figure class=&quot;imageblock widthContent&quot; data-origin-height=&quot;536&quot; data-origin-width=&quot;1722&quot;><span data-alt=&quot;해리 포터와 죽음의 성물 2 평점&quot; data-phocus=&quot;https://blog.kakaocdn.net/dna/dVQ8p2/btsLz4cfocy/AAAAAAAAAAAAAAAAAAAAAB-EgP__ASbsmBikN8jwYZAWB2UNKHYC0OIKWN6HxVXi/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=jHy7L2IrWY0omnGEohrglc0RRDU%3D&quot; data-url=&quot;https://blog.kakaocdn.net/dna/dVQ8p2/btsLz4cfocy/AAAAAAAAAAAAAAAAAAAAAB-EgP__ASbsmBikN8jwYZAWB2UNKHYC0OIKWN6HxVXi/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=jHy7L2IrWY0omnGEohrglc0RRDU%3D&quot;></span><figcaption>해리 포터와 죽음의 성물 2 평점</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 96%, 관객 평점 89%입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>90점대의 높은 평론가 평점과 더불어 <b>로튼 토마토 신선도 보증 마크 또한 획득하며 좋은 평가</b>를 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 평론가들의 좋은 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;https://ambitstock.com/entry/%ED%95%B4%EB%A6%AC-%ED%8F%AC%ED%84%B0%EC%99%80-%EC%A3%BD%EC%9D%8C%EC%9D%98-%EC%84%B1%EB%AC%BC-1-%EA%B2%B0%EB%A7%90-%ED%9B%84%EA%B8%B0-%EB%8B%A4%EC%8B%9C%EB%B3%B4%EA%B8%B0-%EC%8B%9C%EB%A6%AC%EC%A6%88-%EB%AA%A8%EB%93%A0-%EC%9E%91%ED%92%88-%EC%A4%91-%EC%9D%B8%EA%B8%B0-%EC%88%9C%EC%9C%84%EB%8A%94&quot; rel=&quot;noopener&quot; title=&quot;해리 포터와 죽음의 성물 1편&quot;>해리 포터와 죽음의 성물 1편의 평점은?</a></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%ED%95%B4%EB%A6%AC-%ED%8F%AC%ED%84%B0%EC%99%80-%EC%A3%BD%EC%9D%8C%EC%9D%98-%EC%84%B1%EB%AC%BC-1-%EA%B2%B0%EB%A7%90-%ED%9B%84%EA%B8%B0-%EB%8B%A4%EC%8B%9C%EB%B3%B4%EA%B8%B0-%EC%8B%9C%EB%A6%AC%EC%A6%88-%EB%AA%A8%EB%93%A0-%EC%9E%91%ED%92%88-%EC%A4%91-%EC%9D%B8%EA%B8%B0-%EC%88%9C%EC%9C%84%EB%8A%94',
      text: '해리 포터와 죽음의 성물 1 결말 후기 다시보기 < 시리즈 모든 작품 중 인기 순위는 ? >'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '해리 포터와 죽음의 성물 2 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>해리 포터와 죽음의 성물 2 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><b>Don Shanahan - 평점(오리지널 스코어: 5/5)</b><br>해리 포터와 죽음의 성물 2는 시리즈의 마지막 장면을 결합하여, 그 자체로 영화 역사에 독특한 위치를 차지할 만한 서사적 즐거움을 선사한다. 이 영화는 모든 노력의 결실을 보여주며, 그 자체로 거대한 엔터테인먼트를 전달한다.</br></p>'
    },
    {
      type: 'body',
      html: '<p><b>Martin Carr - 평점(오리지널 스코어: 8/10)</b><br/>해리 포터와 죽음의 성물 2는 해리의 이야기를 완벽하게 엮어내며, 감동과 액션을 결합해 관객들에게 강렬한 인상을 남긴다. 이 영화는 시리즈를 아름답게 마무리 지으며, 팬들에게 큰 기쁨을 선사한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Sean Axmaker - 평점(오리지널 스코어 없음)</b><br/>데이비드 예이츠 감독은 해리 포터 시리즈에서 비주얼적 즐거움이나 스펙터클한 장면보다, 캐릭터들의 관계와 그들의 발전에 중점을 두었다. 그의 섬세한 접근 방식은 영화 전반에 걸쳐 감동을 더한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Brian Eggert - 평점(오리지널 스코어: 3.5/4)</b><br/>영화 제작자들은 이 마지막 후속 편뿐만 아니라 시리즈 전체에서 무엇인가 특별한 것을 만들어냈다. 해리 포터와 죽음의 성물 2는 그 자체로도 뛰어난 작품이지만, 전체적인 시리즈의 완성도를 높여준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Cory Woodroof - 평점(오리지널 스코어 없음)</b><br/>해리 포터와 죽음의 성물 2는 위대한 프랜차이즈의 만족스러운 결말을 선사한다. 이 영화는 팬들에게 깊은 감동을 주며, 시리즈의 끝을 잘 마무리한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>David Nusair - 평점(오리지널 스코어: 3.5/4)</b><br/>해리 포터와 죽음의 성물 2는 시리즈 중에서 가장 뛰어난 작품이다. 이 영화는 모든 면에서 최고를 자랑하며, 팬들에게 최고의 경험을 선사한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mike Massie - 평점(오리지널 스코어: 8/10)</b><br/>새로운 장소들은 광대하고 놀라울 정도로, 세트는 아름답게 장식되어 있으며, 등장하는 생물들은 더욱 거대하고 인상 깊다. 해리 포터와 죽음의 성물 2는 시각적, 감각적으로 풍성한 경험을 선사한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Richard Propes - 평점(오리지널 스코어: 3.5/4.0)</b><br/>이 영화에는 크고 작은 순간들이 가득하며, 그 모든 순간들이 관객을 숨죽이게 만든다. 해리 포터와 죽음의 성물 2는 여러 가지 감정을 불러일으키는 작품이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Kate Rodger - 평점(오리지널 스코어: 4/5)</b><br/>영화는 장엄한 3D 대결을 통해 강렬한 감정을 전달하며, 출연진과 제작진은 팬들에게 자랑스럽게 마지막 인사를 할 자격이 있다. 해리 포터와 죽음의 성물 2는 감동적이고 뛰어난 결말을 선사한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Leslie Combemale - 평점(오리지널 스코어 없음)</b><br/>이 영화는 시리즈를 아름답게 마무리 지으며, 원작 소설을 깊이 존중하는 모습을 보여준다. 팬들은 이 영화를 통해 해리 포터 세계관의 끝을 감동적으로 경험할 수 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Guillem Martinez Oya - 평점(오리지널 스코어: 5/5)</b><br/>해리 포터와 죽음의 성물 2는 십 대 관객을 대상으로 한 좋은 영화로, 마법이 가르쳐지는 것뿐만 아니라, 영화 내내 재현되는 영국의 고정관념들이 그 영향을 미친다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mattie Lucas - 평점(오리지널 스코어: 3.5/4)</b><br/>데이비드 예이츠 감독은 이 영화에서 감동적이고 유머러스한 요소와 함께 극적인 액션을 완벽하게 조화시킨다. 해리 포터와 죽음의 성물 2는 감정과 흥분을 균형 있게 다룬다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Micheal Compton - 평점(오리지널 스코어 없음)</b><br/>해리 포터와 죽음의 성물 2는 사랑받는 프랜차이즈의 마지막 장을 장식하며, 시리즈의 약간 더 나은 이전 작품에서 약속된 바를 완전히 실현하지는 못했지만, 만족스러운 결말을 선사한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Rachel Wagner - 평점(오리지널 스코어: A)</b><br/>책과는 많은 차이가 있지만, 나는 이 영화를 좋아한다. 해리 포터와 죽음의 성물 2는 시리즈의 끝을 매우 만족스럽게 마무리 짓는다.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화  로튼 토마토 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'body',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'ending',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%ED%81%AC%EB%A6%AC%EC%8A%A4%ED%86%A0%ED%8D%BC-%EB%86%80%EB%9E%80-%EB%AA%A8%EB%93%A0-%EC%9E%91%ED%92%88-%EB%A1%9C%ED%8A%BC-%ED%86%A0%EB%A7%88%ED%86%A0-%ED%95%B4%EC%99%B8-%ED%8F%89%EB%A1%A0%EA%B0%80-%ED%8F%89%EC%A0%90-%EC%88%9C%EC%9C%84-%EC%88%9C%EC%84%9C%EB%8C%80%EB%A1%9C-%EC%B4%9D%EC%A0%95%EB%A6%AC',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%9D%B4%EB%8F%99%EC%A7%84-%ED%8F%89%EB%A1%A0%EA%B0%80%EA%B0%80-%EC%B6%94%EC%B2%9C%ED%95%98%EB%8A%94-%EB%B3%84-%EB%8B%A4%EC%84%AF%EA%B0%9C-%EB%AA%85%EC%9E%91-%EB%B3%BC%EB%A7%8C%ED%95%9C-%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-TOP-10',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%EB%B0%95%ED%8F%89%EC%8B%9D-%ED%8F%89%EB%A1%A0%EA%B0%80-%EB%AA%85%EC%9E%91-10%EC%84%A0',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EB%A7%88%EB%B8%94-%EC%98%81%ED%99%94-%EA%B0%9C%EB%B4%89-%EC%98%88%EC%A0%95-%EC%B4%9D-%EC%A0%95%EB%A6%AC-2025%EB%85%84-2027%EB%85%84',
      text: '마블 영화 개봉 예정 총 정리 2025년 - 2027년'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%ED%95%B4%EB%A6%AC%ED%8F%AC%ED%84%B0-%EC%8B%9C%EB%A6%AC%EC%A6%88-%EC%9D%B8%EA%B8%B0-%EC%88%9C%EC%9C%84-%EC%88%9C%EC%84%9C-%EC%B6%94%EC%B2%9C-%ED%8F%89%EC%A0%90-TOP-8',
      text: '해리포터 시리즈 인기 순위 순서 추천 평점 TOP 8'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
