const post = {
  id: 167,
  sections: [
    {
      type: 'intro',
      html: '<p>호리아 개봉 정보 출연진 관람평 후기 포토 해외 평점 반응 뜻 < 프랑스 영화 44회 카이로 국제 영화제 특별 언급 작품 ></p>'
    },
    {
      type: 'image',
      src: '/images/post167_thumb.svg',
      alt: '호리아 개봉 정보 출연진 관람평 후기 포토 해외 평점 반응 뜻 < 프랑스 영화 44회 카이로 국제 영화제 특별 언급 작품 >',
      caption: '호리아 개봉 정보 출연진 관람평 후기 포토 해외 평점 반응 뜻 < 프랑스 영화 44회 카이로 국제 영화제 특별 언급 작품 >'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: '/netflix-movie-recommend-guide-top-10/',
      text: '넷플릭스 영화 추천 2024년 9월 인기 작품 순위 TOP 10 총정리!'
    },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '호리아 개봉 정보 출연진 관람평 후기 포토',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '호리아 기본 정보 출연진',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<h4 data-pm-slice=&quot;1 1 []&quot;><b>영화 제목 : 호리아 Houria, 2024</b></h4>'
    },
    {
      type: 'body',
      html: '<h4><b>개요</b></h4>'
    },
    {
      type: 'body',
      html: '<p>프랑스드라마 98분 12세 이상 관람가</p>'
    },
    {
      type: 'body',
      html: '<h4><b>개봉</b></h4>'
    },
    {
      type: 'body',
      html: '<p>2024.12.26.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>감독</b></h4>'
    },
    {
      type: 'body',
      html: '<p>모니아 메두르</p>'
    },
    {
      type: 'body',
      html: '<h4><b>출연</b></h4>'
    },
    {
      type: 'body',
      html: '<p>리나 쿠드리, 라쉬다 브라크니, 프란시스 니짐, 마르완 파레스</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '호리아 해외 관객 평점',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: #333333; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>호리아 해외 관객 평점입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>해외 관객 평점은 6.4점으로 높지 않은 평점을 기록하였습니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 낮은 평점을 이끌었는지 해외 관객 후기 및 로튼 토마토 평론가 관람평 후기 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '호리아 결말후기 관람평 포토',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>호리아 결말후기 관람평 포토입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><b>Blai Morell - 평점(4/5)</b><br>재건, 저항, 자매애에 대한 이야기를 이번에는 춤을 탈출구이자 자유의 공간으로 삼아 풀어내었다.</br></p>'
    },
    {
      type: 'body',
      html: '<p><b>Luis Martínez - 평점(3/5)</b><br/>관객은 어느 순간 화면에 보이는 것과 하나가 된 듯한 경험을 하게 된다.</p>'
    },
    {
      type: 'body',
      html: '<p><br/><b>richie v - 평점 3/5 프랑스어로 기록된 리뷰</b></p>'
    },
    {
      type: 'body',
      html: '<h4><b>영화 개요</b></h4>'
    },
    {
      type: 'body',
      html: '<p>‘호리아’는 알제리에 사는 젊고 재능 있는 발레리나 호리아가 호텔 청소부로 일하며, 어머니에게 차를 사주기 위해 밤마다 불법 도박에 참여하는 이야기이다.</p>'
    },
    {
      type: 'body',
      html: '<p>어느 날, 도박에서 큰돈을 따지만 참가자 중 한 명에게 공격당한 후 큰 부상을 입고 말까지 잃게 된다.</p>'
    },
    {
      type: 'body',
      html: '<p>이 사건을 계기로 그녀의 삶은 완전히 달라지며, 후리아는 자신과 비슷한 상처를 가진 여성들로 구성된 지원 그룹을 통해 새로운 길을 찾아가게 된다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>비평</b></h4>'
    },
    {
      type: 'body',
      html: '<p>‘호리아’는 감독 무니아 메두르의 이전 작품 ‘파피차, 자유를 꿈꾸며’와 마찬가지로 알제리의 사회적, 정치적 현실 속에서 여성들의 강인함과 연대를 그린 영화이다.</p>'
    },
    {
      type: 'body',
      html: '<p>주인공 후리아는 도전과 역경을 통해 성장하는 여성의 모습과 춤의 열정을 담아내며 관객에게 감동을 전한다.</p>'
    },
    {
      type: 'body',
      html: '<p>그러나 영화는 일부 캐릭터의 반복적인 설정과 약간의 불필요한 요소들로 인해 몰입을 방해하기도 한다.</p>'
    },
    {
      type: 'body',
      html: '<p>특히 소냐가 스페인으로 떠나는 장면은 감정적으로 충분히 설득력이 없었고, 몇몇 등장인물은 매력을 느끼기 어려웠다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>총평</b></h4>'
    },
    {
      type: 'body',
      html: '<p>이 영화는 춤과 음악의 조화가 빚어내는 감동적인 순간들을 선사하며, 여성 간의 연대를 통해 역경을 극복하는 메시지를 담고 있다.</p>'
    },
    {
      type: 'body',
      html: '<p>다소 반복적인 주제와 서사의 약점에도 불구하고, 후리아가 보여주는 성장과 마지막 장면의 빛나는 마무리는 인상적이다.</p>'
    },
    {
      type: 'body',
      html: '<p>관객들에게 평화와 희망을 전달하는 작품이다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '호리아 뜻',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p><b>영화 호리아의 제목 호리아는 주인공인 호리아의 이름을 그대로 차용한 영화 제목입니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 호리아 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
