const post = {
  id: 117,
  sections: [
    {
      type: 'intro',
      html: '<p>카라바조의 그림자 개봉 관람평 결말 후기 뜻 해석 포토 <천재 화가의 작품 뒤에 숨겨진 어둠></p>'
    },
    {
      type: 'image',
      src: '/images/post117_thumb.svg',
      alt: '카라바조의 그림자 개봉 관람평 결말 후기 뜻 해석 포토 <천재 화가의 작품 뒤에 숨겨진 어둠>',
      caption: '카라바조의 그림자 개봉 관람평 결말 후기 뜻 해석 포토 <천재 화가의 작품 뒤에 숨겨진 어둠>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '영화 카라바조의 그림자 개봉',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>프랑스 이탈리아 영화 카라바조의 그림자가 2025년 1월 22일 개봉합니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>이번 포스팅에서는 <span style=&quot;background-color: #9feec3;&quot;><b>영화 제목인 카라바조의 그림자의 뜻 해석 그리고 해외 로튼 토마토 평론가 반응 후기</b></span>를 함께 알아보도록 하겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EB%B3%BC%EB%A7%8C%ED%95%9C-%EC%98%81%ED%99%94-%EB%93%9C%EB%9D%BC%EB%A7%88-%EC%B6%94%EC%B2%9C-%EB%84%B7%ED%94%8C%EB%A6%AD%EC%8A%A4-%EC%8B%9C%EC%B2%AD%EA%B0%80%EB%8A%A5-%EC%9D%B4%EB%8F%99%EC%A7%84-%EB%B0%95%ED%8F%89%EC%8B%9D-%ED%95%9C%EC%A4%84%ED%8F%89-%EC%B6%94%EC%B2%9C%EC%9E%91%ED%92%88',
      text: '볼만한 영화 드라마 추천 넷플릭스 시청가능 <이동진 박평식 한줄평 추천작품>'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%9D%B4%EB%8F%99%EC%A7%84-%ED%8F%89%EB%A1%A0%EA%B0%80%EA%B0%80-%EC%B6%94%EC%B2%9C%ED%95%98%EB%8A%94-%EB%B3%84-%EB%8B%A4%EC%84%AF%EA%B0%9C-%EB%AA%85%EC%9E%91-%EB%B3%BC%EB%A7%8C%ED%95%9C-%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-TOP-10',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: 'https://ambitstock.com/entry/%EC%98%81%ED%99%94-%EC%B6%94%EC%B2%9C-%EB%B0%95%ED%8F%89%EC%8B%9D-%ED%8F%89%EB%A1%A0%EA%B0%80-%EB%AA%85%EC%9E%91-10%EC%84%A0',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '카라바조의 그림자 줄거리',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>성모의 죽음, 메두사, <br>성 마태오의 소명, <br>세례 요한의 참수…</br></br></span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p><br/>이탈리아를 대표하는 천재 화가 ‘카라바조’. 살해 혐의로 도망자 신세가 된 \'카라바조\'는 로마 교외로 도피 생활을 이어가는 와중에도 그림을 놓지 않는다.</p>'
    },
    {
      type: 'body',
      html: '<p>한편, 교황청은 그런 그의 사면 자격을 조사하기 위해 비밀리에 ‘그림자’를 파견해 뒤를 쫓는다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '카라바조의 그림자 영어 제목 뜻',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<blockquote><b><span style=&quot;font-family: \'Noto Serif KR\'; color: #781b33;&quot;>카라바조의 그림자 <br/>Caravaggio\'s Shadow</span></b></blockquote>'
    },
    {
      type: 'body',
      html: '<p><b>카라바조의 그림자 (Caravaggio\'s Shadow)</b>는 <span style=&quot;background-color: #9feec3;&quot;><b>카라바조가 남긴 예술적 유산과, 그의 삶에서 발생한 어두운 면과 그가 겪은 갈등, 그리고 죄와 도피의 그림자까지도 의미하는 표현</b></span>입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>카라바조는 뛰어난 화가로서 인정받았지만, 동시에 살인 혐의로 도망자 신세가 되었고, 그의 생애와 작품에 대한 논란이 많았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>&quot;그림자&quot;는 영화에서 그를 쫒는 이들을 의미하기도 하지만 그의 예술적 영광 뒤에 숨겨진 어두운 면</b></span>, 그리고 그가 교황청과 권력에 의해 추적당하면서 겪은 내적 갈등과 외적 추적을 상징적으로 나타냅니다.</p>'
    },
    {
      type: 'body',
      html: '<p>그의 삶에 얽힌 복잡한 감정과 사회적 압박을 암시하는 제목이라 할 수 있습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '카라바조의 그림자 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: #333333; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>카라바조의 그림자 로튼 토마토 평론가 평점입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><figure class=&quot;imageblock widthContent&quot; data-origin-height=&quot;369&quot; data-origin-width=&quot;1120&quot;><span data-alt=&quot;카라바조의 그림자 로튼 토마토 평점&quot; data-phocus=&quot;https://blog.kakaocdn.net/dna/bm64Q3/btsLBjGPKS5/AAAAAAAAAAAAAAAAAAAAAO2RMNdUXyU3pLShE7CeOxD8kh9CLQZbL_fpG1f2yvlc/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=6HJf74HUoFxlxEZ6tHGh%2B75ZWZk%3D&quot; data-url=&quot;https://blog.kakaocdn.net/dna/bm64Q3/btsLBjGPKS5/AAAAAAAAAAAAAAAAAAAAAO2RMNdUXyU3pLShE7CeOxD8kh9CLQZbL_fpG1f2yvlc/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=6HJf74HUoFxlxEZ6tHGh%2B75ZWZk%3D&quot;></span><figcaption>카라바조의 그림자 로튼 토마토 평점</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점은 아직 기록되지 않았고, 관객 평점 56%입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>평론가 후기는 3개 등록되어 있으며 포스팅 하단에서 다뤄보도록 하겠습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>IMDB에서의 평점도 6.5점으로 로튼 토마토와 마찬가지로 낮은 시청자 평점을 기록하였습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 <span style=&quot;background-color: #9feec3;&quot;><b>해외 관객들에게 좋지 못한 평점을 이끌었는지 후기 관람평</b></span> 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '카라바조의 그림자 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>카라바조의 그림자 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><b>Stephen Romei - 평점 4/5 (오리지널 스코어)</b><br/><span style=&quot;background-color: #9feec3;&quot;><b>이 몰입감 있는 영화는 역사적이고 시의적절하다. </b></span></p>'
    },
    {
      type: 'body',
      html: '<p>오늘날 취소 문화에 휘말린 다양한 예술가들 사이에서, 이 17세기 이야기는 여전히 중요한 메시지를 담고 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Andiee Paviour - 평점 3.5/5 (오리지널 스코어)</b><br/><span style=&quot;background-color: #9feec3;&quot;><b>화가 카라바조에게 예술은 1600년대에서 불타오르는 삶의 방식이다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><b>Mark Demetrius - 평점 17/20 (오리지널 스코어)</b><br/>...신나는 이야기이며, 카라바조에 대해 잘 알지 못한 사람들에게는 더욱 매력적으로 다가갈 것이다.</p>'
    },
    {
      type: 'h2',
      id: 'section7',
      text: '카라바조의 그림자 해외 관객 반응 후기',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<h3>6/10감동적인 그림</h3>'
    },
    {
      type: 'body',
      html: '<p>이 영화는 16세기 이탈리아 화가 미켈란젤로 메리시 다 카라바조의 이야기를 교황의 조사관의 시각으로 풀어낸 작품이다. 그는 &quot;보는 대로 그린&quot; 화가로 유명했으며, 신성한 것과 가까운 현실을 그리려고 했고, 그로 인해 카톨릭 교회와는 맞지 않았다.</p>'
    },
    {
      type: 'body',
      html: '<p>그의 작품 속에는 마리아나 성 베드로와 같은 유명한 성인들이 prostitute, 빈민 등으로 묘사되었다. 그의 삶에는 여러 전설이 얽혀 있으며, 성직자들 사이에서 성별에 관계없이 성적 관계를 맺은 것으로 알려져 있다.</p>'
    },
    {
      type: 'body',
      html: '<p>그의 경쟁자들은 이를 이유로 그를 파멸시키려 했다.</p>'
    },
    {
      type: 'body',
      html: '<p>그는 사람을 죽인 후(실수였을까?) 로마를 떠나 네이플스, 시칠리아, 몰타 등지로 도망쳤다.</p>'
    },
    {
      type: 'body',
      html: '<p>영화의 이야기는 다소 길지만, 이 영화가 왜 봐야 하는지에 대한 이유는 그다지 중요하지 않다.</p>'
    },
    {
      type: 'body',
      html: '<p>영화 전체가 마치 그림처럼 연출되었으며, 16세기 이탈리아 회화에서 느낄 수 있는 분위기가 있다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>다시 볼 것인가? 아니, <br/>하지만 보고 나서 만족스럽다.</span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<h3>7/10예술 애호가를 위한 영화</h3>'
    },
    {
      type: 'body',
      html: '<p>이 영화를 완전히 이해하려면 카라바조의 작품을 좋아하는 것이 도움이 된다.</p>'
    },
    {
      type: 'body',
      html: '<p>그의 선택과 그로 인해 논란이 된 작품들이 왜 그렇게 훌륭했는지 이해하는 데 도움이 된다. 당시 카톨릭 교회는 유일한 교회였지만, 프로테스탄트 교회가 성장하며 바티칸은 긴장감을 느꼈다.</p>'
    },
    {
      type: 'body',
      html: '<p>평범한 사람들의 현실과 성인들의 거룩한 표현 사이의 갈등은 많은 수익을 내지 못했다.</p>'
    },
    {
      type: 'body',
      html: '<p>카라바조의 작품을 알고 있으면 이 영화의 많은 장면들을 이해하는 데 도움이 된다.</p>'
    },
    {
      type: 'body',
      html: '<p>그의 작품이 훌륭한 이유는 당시에는 상상할 수 없었던 방식으로 일상적인 사람들의 얼굴을 성인이나 천사의 얼굴로 사용했기 때문이다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>당시의 신은 성별도, <br/>성적 정체성도 없었다.</span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<h3>6/10지루하고 서사적인 섬세함이 부족하다</h3>'
    },
    {
      type: 'body',
      html: '<p>주요 교회 인물들이 화가의 관능성은 비난하면서도 그의 작품을 축적하려는 위선적인 태도는 이상하게 보인다.</p>'
    },
    {
      type: 'body',
      html: '<p>카라바조는 세속적인 시대에서 매력적이고 동정적인 혁명적 영웅으로 등장한다.</p>'
    },
    {
      type: 'body',
      html: '<p>그의 동물적인 성적 행위는 불쾌하지만, 그가 한 최고의 대사들은 기억에 남는다.</p>'
    },
    {
      type: 'body',
      html: '<p>증인들을 인터뷰하며 그의 임무에 몰두하는 조사관은 단조로워지고, 영화는 서사적인 섬세함이 부족해 점차 지루해진다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>예술가의 강렬한 독창성이 <br/>더 분명히 드러났다면 <br/>더 즐겁게 느껴졌을 것이다.</span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 카라바조의 그림자 해외 평론가 관객 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
