const post = {
  id: 147,
  sections: [
    {
      type: 'intro',
      html: '<p>해리 포터와 죽음의 성물 1 결말 후기 다시보기 < 시리즈 모든 작품 중 인기 순위는 ? ></p>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '해리 포터와 죽음의 성물 1 재개봉 결말 후기 다시 보기',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '해리 포터와 죽음의 성물 1 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>해리 포터와 죽음의 성물 1 로튼 토마토 평론가 평점입니다.</b></span><span style=&quot;background-color: #f3c000;&quot;></span></p>'
    },
    { type: 'image', src: '/images/post147_img1.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><figure><figcaption>해리 포터와 죽음의 성물 1 로튼 토마토 평점</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 77%, 관객 평점 85%입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>기대한 바에 못 미치는 평론가 평점으로 관객 평점에 비해 낮은 평론가 평점을 기록하였습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>하지만 <b>로튼 토마토 신선도 보증 마크를 획득하며 작품성면에서 긍정적인 평가를 받았습니다.</b></p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 이러한 평론가들의 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>해리포터 시리즈 모든 작품의 평점 순위는 포스팅 하단에서 확인 가능합니다.</b></p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '해리 포터와 죽음의 성물 1 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>해리 포터와 죽음의 성물 1 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post147_img9.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post147_img8.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post147_img7.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post147_img6.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post147_img5.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post147_img4.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post147_img3.jpg', alt: '해리 포터와 죽음의 성물 1', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post147_img2.jpg', alt: '해리 포터와 죽음의 성물 1 로튼 토마토 평점', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><b>Don Shanahan - 평점 없음</b><br>영화의 모든 노력에서 오는 즐거움을 느낄 수 있을 정도로, 시리즈의 마지막 장을 구성한 두 영화는 영화 역사에서 그만한 자리를 차지할 만한 서사적 오락을 전달하였다.</br></p>'
    },
    {
      type: 'body',
      html: '<p><b>Martin Carr - 평점(8/10)</b><br/>탁월한 연기와 결점 없는 구조 속에서, 해리 포터와 죽음의 성물 1은 인간관계에 대한 집착을 탐구한다.</p>'
    },
    {
      type: 'body',
      html: '<p>이 영화는 우리에게 사랑하는 사람들과의 시간이 결국 유한하다는 사실을 상기시킨다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Brian Eggert - 평점(3/4)</b><br/>놀라울 정도로 드라마틱한 힘과 비전형적인 표현 방식으로, 해리 포터와 죽음의 성물 1은 관객들에게 지금까지 야츠 감독이 프랜차이즈를 이끌어 온 공식적인 방식에 대해 재고하도록 도전할 것이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>David Nusair - 평점(2.5/4)</b><br/>불행히도 일관되지 않은 각색으로, 전반적으로 한 시간 정도 더 짧았어야 한다는 느낌을 준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>James McMahon - 평점(4/5)</b><br/>과장된 표현? 지나치게 들뜬? 그런 말을 하지 마라.</p>'
    },
    {
      type: 'body',
      html: '<p><b>해리 포터의 세계에서는 냉소적인 태도를 가질 여지가 없다. </b></p>'
    },
    {
      type: 'body',
      html: '<p>게다가 영화관에 들어간 지 두 시간 반 후에 자신이 어떤 말을 하고 있는지 봐라...</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mike Massie - 평점(5/10)</b><br/>이전 영화들보다 더 많이, 이 영화는 이야기를 부분적으로만 담고 있어 그 자체로 영화의 일부에 불과하다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Richard Propes - 평점(3.0/4.0)</b><br/>다니엘 래드클리프는 어색한 소년에서 상당히 훌륭한 젊은 남성으로 변신하였다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>이는 해리 포터 자신과 마찬가지이다.</b></p>'
    },
    {
      type: 'body',
      html: '<p><b>Nikhat Kazmi - 평점 없음</b><br/>죽음의 성물 1의 첫 번째 파트는 실제로 폭풍 전의 낮은 소리와 같다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Sarah Knight Adamson - 평점(4/4)</b><br/>감동을 받지 않을 수 없다. 이 영화는 나를 진정한 공포로 눈물짓게 만들었다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Josué Corro - 평점 없음</b><br/>액션이 부족하지만, 이 결핍을 세 명의 주인공 간의 관계에 대한 친밀한 시선으로 보완하는 영화이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Luis Miguel Cruz - 평점(3.5/5)</b><br/>이 영화는 관객들이 마지막 파트에 대한 기대감을 불러일으키는 임무를 완수한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mariana Tinoco Rivera - 평점(3.5/5)</b><br/>데이비드 야츠는 팬들이 시리즈의 마지막 편을 기대하게 만드는 데 성공하였다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Juan Pablo Russo - 평점(4/10)</b><br/>긴 시간과 지루한 이야기로, 하나의 영화로도 충분히 전할 수 있었을 것이다.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 해리 포터와 죽음의 성물 1 로튼 토마토 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'body',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'ending',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: '/series-recommend-rating-top-8/',
      text: '해리포터 시리즈 인기 순위 순서 추천 평점 TOP 8'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/movie-recommend-critic-masterpiece/',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'cta',
      href: '/marvel-movie-release-upcoming/',
      text: '마블 영화 개봉 예정 총 정리 2025년 - 2027년'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' }
  ]
}

module.exports = post
