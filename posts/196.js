const post = {
  id: 196,
  sections: [
    {
      type: 'intro',
      html: '<p><b>2024년 11월 27일 개봉하는 모아나 2 시사회 후기 관람평입니다.</b></p><p>디즈니의 애니메이션 스튜디오에서 일부 미리 공개된 모아나 2는 현재 긍정적인 평가와 더불어 작품성에 대한 걱정을 불러일으키고 있습니다.</p><p><span style=&quot;background-color: #f3c000;&quot;><b>과연 어떠한 평가를 받았는지 영화 개봉전 미리보는 시사회, 모아나 2 후기 관람평</b></span> 확인해 보도록 하겠습니다.</p>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post196_img1.jpg', alt: '모아나 2', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/review-critic-reaction-release-upcoming-movie/',
      text: '글래디에이터 Ⅱ 2 시사회 후기 평론가 관람평 반응 11월 가장 기대되는 개봉 예정작 영화'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '모아나 제목 의미 뜻',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>모아나(Moana)<br/></span>하와이어 - 깊고 넓은 바다</b></span></blockquote>'
    },
    { type: 'image', src: '/images/post196_img2.jpg', alt: '모아나 2', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>영화 모아나(Moana)의 제목은 주인공의 이름이자 하와이어로 &quot;바다&quot;를 의미합니다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>폴리네시아 언어권에서 &quot;깊고 넓은 바다&quot;를 상징</b></span>하며, 모아나의 캐릭터와 그녀의 여정에서 중요한 의미를 지닙니다.</p>'
    },
    {
      type: 'body',
      html: '<p>주인공 모아나는 바다와의 깊은 연결고리를 가진 인물로, 영화는 그녀가 자신과 바다의 운명을 발견하며 성장하는 과정을 그리고 있습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>또한, 제목은 영화의 주요 테마인 모험, 자기 발견, 자연과 인간의 관계를 상징적으로 표현합니다.</p>'
    },
    {
      type: 'body',
      html: '<p>바다(모아나)는 모험과 변화의 공간일 뿐만 아니라 주인공이 자신의 정체성과 사명을 찾는 과정에서 스승 같은 역할을 합니다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '모아나 2 시사회 후기 긍정적 반응',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>모아나 2 시사회 후기 긍정적 반응입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post196_img3.jpg', alt: '모아나 2', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>영화 Moana 2는 디즈니의 애니메이션 스튜디오에서 일부 미리 공개되었으며, <b>여러 평론가들은 첫 작품의 전통을 잘 계승</b>하면서도 새로운 요소를 성공적으로 결합했다고 평가하였습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>특히 음악에 대한 평가</b>가 돋보였는데, <span style=&quot;background-color: #9feec3;&quot;><b>첫 영화의 상징적인 사운드트랙을 재해석하면서 새로운 활력을 더한 것이 인상적</b></span>이었다고 언급되었습니다.</p>'
    },
    {
      type: 'body',
      html: '<div><center><a href=&quot;https://ambitstock.com/332&quot; title=&quot;모아나 2 로튼 평론가 후기 보러가기&quot;><button class=&quot;primary&quot;>모아나 2 로튼 평론가 후기 보러가기</button></a></center></div>'
    },
    {
      type: 'body',
      html: '<p>새로운 작곡가들이 추가되며 주인공 모아나의 성장과 그녀의 여정에 공감할 수 있는 감동적인 곡들이 포함되었다는 평이 많았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>이 외에도 주인공의 더 성숙한 캐릭터와 동생 ‘시메아’의 등장으로 영화의 감정적 무게감이 더욱 깊어졌다는 점이 긍정적으로 평가</b>되었습니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '모아나 2 시사회 후기 부정적 반응',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>모아나 2 시사회 후기 부정적 반응입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post196_img6.jpg', alt: '모아나 2', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post196_img5.jpg', alt: '모아나 2', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post196_img4.jpg', alt: '모아나 2', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>영화 제작 과정에서 초기에는 <b>디즈니+ 시리즈로 기획되었다가 극장용 영화로 전환된 점</b>이 일부 팬들과 평론가들에게 불안 요소로 작용하였습니다. 또한, 디즈니가 <b>영화의 리뷰 엠바고를 개봉 하루 전까지 유지하기로 결정하면서 영화에 대한 자신감이 부족한 것이 아니냐는 의문</b>이 제기되었습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이는 디즈니의 최근 전략 변화와 연관될 수 있지만, 일부에서는 영화의 완성도와 흥행 가능성을 의심하게 만드는 요소로 보고 있습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 모아나 2 에 대한 개봉 전 시사회 해외 반응 전해드렸습니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>양화 모아나 2는 2024년 11월 27일 영화관에서 확인가능 합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' }
  ]
}

module.exports = post
