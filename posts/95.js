const post = {
  id: 95,
  sections: [
    {
      type: 'intro',
      html: '<p><b>영화 미키17 (영어제목 : Mickey 17 )이 드디어 개봉합니다. <br>2025년 2월 28일 개봉예정작으로 믿고 보는 봉준호감독님의 연출과 로버트 패틴슨 배우의 엄청난 연기력을 확인해 보실 수 있습니다.</br></b></p><p>이번 포스팅에서는 <span style=&quot;background-color: #9feec3;&quot;><b>영화 미키17 해외 평점과 후기를 알아보고 볼만한 작품인지 확인</b></span>해 판단할 수 있도록 도와드리겠습니다.</p><div><center><a href=&quot;https://search.naver.com/search.naver?sm=tab_hty.top&amp;where=nexearch&amp;ssc=tab.nx.all&amp;query=미키+17+상영일정&amp;oquery=미키+17+상영일정&amp;tqi=iJkOhwqpsECssKoSl14ssssstz0-111019&quot; title=&quot;미키 17 영화 보러가기&quot;><button class=&quot;btn-hover color-8&quot;> 미키 17 바로 보러 가기👉</button></a></center></div>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    { type: 'image', src: '/images/post95_img1.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    {
      type: 'h2',
      id: 'section2',
      text: '영화 미키 17 줄거리',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p>위험한 일에 투입되는 소모품(익스펜더블)으로, 죽으면 다시 프린트되는 ‘미키’가 17번째 죽음의 위기를 겪던 중, <br>그가 죽은 줄 알고 ‘미키 18’이 프린트되면서 벌어지는 예측불허의 이야기를 그리는 영화</br></p>'
    },
    { type: 'image', src: '/images/post95_img2.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    {
      type: 'h2',
      id: 'section3',
      text: '영어제목 뜻 해석',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p>영화 제목 &quot;미키 17&quot;에서 &quot;미키&quot;는 주인공의 이름을 의미하며, &quot;17&quot;은 숫자로, 이 영화의 중요한 설정과 관련이 있다.</p>'
    },
    { type: 'image', src: '/images/post95_img3.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>&quot;미키 17&quot;에서 &quot;17&quot;은 주인공이 반복적으로 죽고 다시 태어나는 과정에서의 번호를 나타내는 것으로, \'미키\'라는 캐릭터가 여러 번 죽고 복제되어 다시 살아나는 상황을 묘사하는 데 중요한 역할을 한다. 이 숫자는 미키라는 인물이 끊임없이 재탄생하는 반복적인 구조를 암시하며, 영화 내에서 인간성과 존재에 대한 질문을 던진다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;font-family: \'Noto Serif KR\';&quot;>&quot;미키 17&quot;은 미키라는 캐릭터가 <br/>17번째로 등장하는 버전이라는 의미</span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>영화는 이러한 복제와 죽음을 반복하는 미키의 이야기를 통해 인간 존재와 삶의 의미, 그리고 반복되는 상황 속에서의 자아와 정체성에 대해 탐구한다.</p>'
    },
    {
      type: 'body',
      html: '<div><center><a href=&quot;https://www.rottentomatoes.com/m/mickey_17&quot; title=&quot;미키 17 해외 평점은?&quot;><button class=&quot;btn-hover color-8&quot;> 해외 평점 보러 가기 👉</button></a></center></div>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>영화 미키 17 평점이 궁금하시다면??</b></p>'
    },
    {
      type: 'body',
      html: '<div><center><a href=&quot;https://search.naver.com/search.naver?sm=tab_hty.top&amp;where=nexearch&amp;ssc=tab.nx.all&amp;query=미키+17+상영일정&amp;oquery=미키+17+상영일정&amp;tqi=iJkOhwqpsECssKoSl14ssssstz0-111019&quot; title=&quot;미키 17 보러가기&quot;><button class=&quot;btn-hover color-8&quot;> 미키 17 바로 보러 가기👉</button></a></center></div>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>위 링크를 통해</b></p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><b>바로 시청하러 가보세요</b></p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '미키 17 해외 평론가 후기',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>이어서 영화 미키 17의 해외 평론가 후기입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post95_img4.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>내용에 스포는 크게 없기에 바로 이어서 확인해 보시길 바랍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>긍정적인 평가와 부정적인 평가를 나눠서 분류하였고, 내용과 함께 평점을 확인해 보실수 있습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>바로 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '미키 17 해외 평론가 긍정적인 평가 후기',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p><b>Wendy Lee Szany - 평점(3/5)</b><br/>봉준호 감독의 가장 기괴하고 Wild한 다크 코미디가 자본주의, 부패, 식민지화, 탐욕에 대해 깊이 탐구하였다. 패틴슨은 미키 역에 완벽한 배우였다. 마지막 장면은 조금 길고 불균형하게 느껴졌다.</p>'
    },
    { type: 'image', src: '/images/post95_img5.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><b>Alistair Ryder - 평점(6/10)</b><br/>&quot;스타쉽 트루퍼스&quot;가 패러디로서 너무 직설적이었다면, &quot;미키 17&quot;은 그보다 더 많은 등장인물들이 자신들이 파시즘을 패러디한다고 관객에게 직접 설명하는 작품이다. 이 영화는 그런 점에서 좋다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Serena Seghedoni - 평점(3.5/5)</b><br/>영화는 기본적인 premise와 뛰어난 연기, 그리고 기술적 완성도가 인상적이다. 그럼에도 불구하고 테마의 일관성이 부족해도 여전히 흥미롭고 즐길 수 있는 이야기로 만들어졌다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Andrew J. Salazar - 평점(4.5/5)</b><br/>&quot;기생충&quot;이 자본주의가 계급주의와 사회 불평등의 근원이라고 비판했다면, 봉준호의 풍자적인 &quot;미키 17&quot;은 같은 자본주의 탐욕이 인간성을 모두 빼앗은 미래를 웃기면서도 감동적으로 그렸다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Brent Hankins - 평점(9/10)</b><br/>미키 17은 자신의 가치를 주장하는 작은, 고집 센 행동에 대한 이야기이다. 이는 단순히 생존을 넘어, 물리적으로, 철학적으로 살아남으며 지워지지 않기를 거부하는 이야기이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Alex Billington - 평점(9/10)</b><br/>이 영화는 봉준호 감독의 &quot;스타쉽 트루퍼스&quot;이다. 아주 멋지며, &quot;설국열차&quot;보다 더 뛰어나다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Germain Lussier - 평점 없음</b><br/>&quot;미키 17&quot;은 불균형하면서도 즐거운 영화이다. 시간이 지나고 나면 더 나아질 가능성이 크다. 지금은 그 미래와 현실을 구분하기에는 너무 이른 시점이다. 하지만 그만큼 기괴한 매력이 있어 한 번은 볼 가치가 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Helen O\'Hara - 평점(4/5)</b><br/>미키처럼 이 영화는 유머러스하고 조금 불규칙적이지만, 동시에 재미있고 사려 깊으며 우리가 생각한 것보다 더 그럴듯하다. 이런 이상한 시대에 잘 맞는 매력적인 우주 oddity이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Savina Petkova - 평점(B)</b><br/>폭력과 착취의 악순환을 끊을 수 있는 두 가지는 사랑과 혁명이다. 다행히 &quot;미키 17&quot;은 두 가지 모두를 제공한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>BJ Colangelo - 평점(9/10)</b><br/>&quot;미키 17&quot;은 자본주의, 식민지화, 부패에 대한 진지하고도 불편하게 웃긴 성찰이다. 이는 우리 시대에 꼭 맞는 영화이며, 봉준호 감독의 최고의 영어 영화이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Kristy Puchko - 평점 없음</b><br/>로버트 패틴슨은 &quot;미키 17&quot;에서 &quot;잭애스&quot;의 매력을 발산한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Lou Thomas - 평점(4/5)</b><br/>&quot;미키 17&quot;은 봉준호 감독의 이전 작품들이 지닌 정치적 날카로움이 부족할 수 있으나, 분명히 매우 재미있고, 큰 스펙터클로 대부분의 블록버스터 영화들과는 차별화된다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Tim Grierson - 평점 없음</b><br/>패틴슨은 미키들을 연기하며 즐기지만, &quot;미키 17&quot;에서 그의 연기가 이 SF 영화의 감동을 더한다. 죽음을 반복하며 주인공은 자신의 영혼이 살아남기를 바란다. 패틴슨은 처음부터 그 영혼을 찾아낸다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Rachel Leishman - 평점(4.5/5)</b><br/>이 영화는 가장 좋은 방식으로 기괴하다. 신선하고, 매력적이며, 아름답고, 당신을 긴장감 넘치게 만든다. 매 순간이 즐겁고, 여러 명의 로버트 패틴슨이 뛰어다니는 것도 큰 매력이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Ben Croll - 평점 없음</b><br/>청춘 아이돌에서 감독의 애지중지하는 존재로, 액션 리더로 성장한 패틴슨은 여기서 코미디가 진정한 소명임을 쉽게 알 수 있다. 그는 젊은 시절 짐 캐리처럼 탄력 있는 신체 연기를 선보인다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>David Opie - 평점(5/5)</b><br/>&quot;미키 17&quot;은 로버트 패틴슨의 최고의 역할을 담고 있으며, 봉준호 감독을 21세기 가장 강력하고 꾸준한 감독으로 자리매김하게 한다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Pete Hammond - 평점 없음</b><br/>세상을 멈추고 내려오고 싶은 이들에게는 이 영화가 적합하다. 죽음을 넘나드는, 어지럽고, 짜릿한 롤러코스터 같은 영화이다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '미키17 해외 평론가 부정적인 평가 후기',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p><b>Hugh Montgomery - 평점(2/5)</b><br/>안타깝게도 이 영화는 어떤 접근법을 취할지 확실하지 않다. 출시가 지연된 이유가 될 수도 있다. 전반적으로 감독에게 심각한 실망감을 안겨주는 작품이다.</p>'
    },
    { type: 'image', src: '/images/post95_img6.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    {
      type: 'h2',
      id: 'section7',
      text: '미키 17 전반적인 평가',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p>지금 확인해 보신 후기만을 보더라도 해외 평론가들의 부정적인 평가보다는 긍정적인 평가가 주를 이루고 있음을 알 수 있습니다.</p>'
    },
    { type: 'image', src: '/images/post95_img8.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post95_img7.jpg', alt: '미키 17', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p>모두가 기다리던 2025년의 대작 \'미키17\'! 영화관에서 꼭 확인해보시길 바랍니다.</p>'
    },
    {
      type: 'ending',
      html: '<div><center><a href=&quot;https://search.naver.com/search.naver?sm=tab_hty.top&amp;where=nexearch&amp;ssc=tab.nx.all&amp;query=미키+17+상영일정&amp;oquery=미키+17+상영일정&amp;tqi=iJkOhwqpsECssKoSl14ssssstz0-111019&quot; title=&quot;미키 17 영화 보러가기&quot;><button class=&quot;btn-hover color-8&quot;> 미키 17 바로 보러 가기👉</button></a></center></div>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' }
  ]
}

module.exports = post
