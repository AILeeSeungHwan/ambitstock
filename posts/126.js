const post = {
  id: 126,
  sections: [
    {
      type: 'intro',
      html: '<p>볼만한 영화 드라마 추천 넷플릭스 시청가능 <이동진 박평식 한줄평 추천작품></p>'
    },
    {
      type: 'image',
      src: '/images/post126_thumb.svg',
      alt: '볼만한 영화 드라마 추천 넷플릭스 시청가능 <이동진 박평식 한줄평 추천작품>',
      caption: '볼만한 영화 드라마 추천 넷플릭스 시청가능 <이동진 박평식 한줄평 추천작품>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '볼만한 영화 드라마 추천',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>볼만한 영화 드라마를 찾으시나요</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>이 포스팅에서 찾고자 하는 영화를 반드시 찾으실 수 있으실 겁니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이동진, 박평식 평론가가 추천하는 영화부터, 넷플릭스에서 시청가능한 영화.</p>'
    },
    {
      type: 'body',
      html: '<p>지브리 스튜디오 모든 작품과 유명 시리즈의 평점 순위 추천과 유명 감독들의 모든 필모 최고작품.</p>'
    },
    {
      type: 'body',
      html: '<p>마지막으로 TVN, JTBC 시청률 순위로 보는 드라마 추천과 2025년 개봉예정작까지, 한 곳에 모아 전해드립니다.</p>'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '1. 이동진 박평식 평론가가 추천하는 영화들',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>첫 번째 볼만한 영화 추천은 평론가가 추천하는 영화들</b></span>입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>우리나라에서 현재 가장 유명한 영화 평론가 딱 두 명을 뽑으라면 이 두 분이 생각날 것입니다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>이동진 평론가<br>박평식 평론가</br></span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>이동진 평론가는 요즘 유튜브를 통해서 젊은 세대에게도 익숙해졌으며, <b>잔잔하고도 깊이 있는 지식을 담아 영화를 리뷰</b>하시는 것으로 유명합니다. 특히나 요즘 개봉하는 모든 영화에 대한 <b>이동진 평론가 한줄평을 검색</b>해보는 유입량이 늘어날 정도로 이동진 평론가의 평가를 궁금해하는 분들이 많은 것 같습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>그리고 박평식 평론가, <b>사악한 촌철살인 한줄평</b>을 남기는 것으로 유명하죠.</p>'
    },
    {
      type: 'body',
      html: '<p>박평식 평론가가 <b>극찬한 영화는 정말 말그대로 명작</b>이라 할 수 있을 정도의 명작으로 유명합니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>이 둘이 추천하는, 이동진 평론가의 평점 만점, 그리고 만점 없는 평론가 박평식의 평점 9점 영화</b>를 모아보았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;이동진 추천 영화&quot;>이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10</a></p>'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;박평식 추천 영화&quot;>박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10</a></p>'
    },
    {
      type: 'cta',
      href: '/movie-recommend-critic-masterpiece/',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '2. 넷플릭스에서 시청가능한 평론가 영화',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p>아무리 좋은 영화를 찾았다고 할지라도 내가 가지고 있는 OTT에서 시청하지 못한다면 도움이 되지 않겠죠.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>넷플릭스에서 시청가능한 이동진, 박평식 평론가의 추천영화</b></span>들을 소개해드립니다.</p>'
    },
    {
      type: 'body',
      html: '<p>현재 2025년 기준 넷플릭스에서 시청하실 수 있으며, 이 페이지를 저장해 두시고 두고두고 꺼내보시길 추천드립니다.</p>'
    },
    {
      type: 'body',
      html: '<p>보장된 재미와 더불어 한층 더 높아진 지식수준과 교양을 탑재하실 수 있으실 겁니다.</p>'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;넷플릭스 시청 가능 이동진 추천 영화&quot;>넷플릭스 영화 추천 | 이동진 평론가 평점 만점작에서 찾는 인생영화 - 넷플릭스 시청가능 수상작품</a></p>'
    },
    {
      type: 'cta',
      href: '/netflix-movie-recommend-critic-rating/',
      text: '넷플릭스 영화 추천 | 이동진 평론가 평점 만점작에서 찾는 인생영화 - 넷플릭스 시청가능 수상'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;넷플릭스 시청 가능 박평식 추천 영화&quot;>넷플릭스 영화 추천 | 박평식 평론가의 7점은 97점이다 명작 영화 평점 순위 TOP 10</a></p>'
    },
    {
      type: 'cta',
      href: '/netflix-movie-recommend-critic-masterpiece-rating-top-10/',
      text: '넷플릭스 영화 추천 | 박평식 평론가의 7점은 97점이다 명작 영화 평점 순위 TOP 10'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '3. TVN JTBC 티빙드라마 추천',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p>영화도 좋지만 길게 볼 수 있는 드라마를 선호하시는 분들도 있을겁니다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>JTBC와 TVN 역대 드라마 시청률을 바탕으로 드라마를 추천해 드립니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>각각 시청률 순위 TOP10으로 선정하였으며, 넷플릭스와 티빙에서 시청가능합니다.</p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;JTBC 드라마 추천&quot;>JTBC 역대 시청률 순위 TOP10 OTT 넷플릭스 드라마 추천</a></p>'
    },
    {
      type: 'cta',
      href: '/netflix-drama-recommend-jtbc-top10-ott/',
      text: 'JTBC 역대 시청률 순위 TOP10 OTT 넷플릭스 드라마 추천'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;TVN 드라마 추천&quot;>2024년 TVN 역대 시청률 TOP 10 티빙 드라마 순위 추천</a></p>'
    },
    {
      type: 'cta',
      href: '/tving-drama-recommend-tvn-top-10/',
      text: '2024년 TVN 역대 시청률 TOP 10 티빙 드라마 순위 추천'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '4. 디즈니 플러스 드라마 추천 - 한국드라마, 해외드라마',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p>디즈니 플러스를 이용하시는 분들을 위해 준비하였습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>디즈니 플러스 한국드라마와 해외드라마를 추천해 드립니다.</b></span> </p>'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;디즈니 플러스 드라마 해외&quot;>디즈니 플러스 볼만한 드라마 추천 순위 2024년 TOP 10 | 해외 편</a></p>'
    },
    {
      type: 'cta',
      href: '/disney-drama-recommend-overseas-top-10/',
      text: '디즈니 플러스 볼만한 드라마 추천 순위 2024년 TOP 10 | 해외편'
    },
    {
      type: 'body',
      html: '<p style=&quot;text-align: center;&quot;><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;디즈니 플러스 드라마 한국&quot;>디즈니플러스 오리지널 시리즈 한국 볼만한 드라마 추천 순위 TOP 10</a></p>'
    },
    {
      type: 'cta',
      href: '/disney-plus-disney-series-drama-recommend-top-10/',
      text: '디즈니플러스 오리지널 시리즈 한국 볼만한 드라마 추천 순위 TOP 10'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '5. 지브리 스튜디오, 유명 시리즈, 감독 필모를 한 번에',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p>모두가 좋아하는 지브리 스튜디오 작품, 해리포터 시리즈와 킹스맨 시리즈.</p>'
    },
    {
      type: 'body',
      html: '<p>그리고 크리스토퍼 놀란 감독과 쿠엔틴 타란티노 감독의 모든 작품을 평점순위로 전해드립니다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>이 포스팅을 통해 어떤 작품이 시리즈, 감독의 최고의 작품인지 확인</b></span>해 보세요.</p>'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot;>지브리 스튜디오 역대 애니메이션 추천 인기 순위 로튼 기준 총정리!</a></p>'
    },
    {
      type: 'cta',
      href: '/animation-recommend-guide/',
      text: '지브리 스튜디오 역대 애니메이션 추천 인기 순위 로튼 기준 총정리!'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot;>해리포터 시리즈 인기 순위 순서 추천 평점 TOP 8</a></p>'
    },
    {
      type: 'cta',
      href: '/series-recommend-rating-top-8/',
      text: '해리포터 시리즈 인기 순위 순서 추천 평점 TOP 8'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot;>킹스맨 시리즈 최고 작품은 무엇일까?</a></p>'
    },
    {
      type: 'cta',
      href: '/series-netflix-movie-recommend/',
      text: '킹스맨 시리즈 최고 작품은 무엇일까? 넷플릭스 영화 추천'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot;>크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!</a></p>'
    },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot;>영화 추천 쿠엔틴타란티노 필모그래피 수상작품 평점 인기 순위 TOP 9</a></p>'
    },
    {
      type: 'cta',
      href: '/movie-recommend-rating-top-9/',
      text: '영화 추천 쿠엔틴타란티노 필모그래피 수상작품 평점 인기 순위 TOP 9'
    },
    {
      type: 'h2',
      id: 'section7',
      text: '6. 기대되는 개봉예정작품은?',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p>2025년 기대되는 개봉예정작품을 추천해 드립니다.</p>'
    },
    {
      type: 'body',
      html: '<p>한국영화부터 해외영화까지, 아직 개봉하지 않은 작품에 대한 해외 시사회 후기 평점을 가져왔습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>자세한 내용은 아래 포스팅에서 확인해 보세요.</p>'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;1월 개봉예정작&quot;>2025년 1월 개봉예정작 총정리! 영화 추천 순위 TOP10</a></p>'
    },
    {
      type: 'cta',
      href: '/release-upcoming-guide-movie-recommend-top10/',
      text: '2025년 1월 개봉예정작 총정리! 영화 추천 순위 TOP10'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;2025년 개봉예정작&quot;> 2025년 한국 넷플릭스 영화 추천 개봉예정작 총정리!</a></p>'
    },
    {
      type: 'cta',
      href: '/netflix-movie-recommend-release-upcoming-guide/',
      text: '2025년 한국 넷플릭스 영화 추천 개봉예정작 총정리!'
    },
    {
      type: 'body',
      html: '<p><a href=&quot;/ rel=&quot;noopener&quot; title=&quot;2025년 현재상영작&quot;>현재상영작 영화 예매 순위 관객수 2025년 1월 TOP10</a></p>'
    },
    {
      type: 'cta',
      href: '/movie-audience-top10-2/',
      text: '현재상영작 영화 예매 순위 관객수 2025년 1월 TOP10'
    },
    {
      type: 'body',
      html: '<p>이상으로 볼만한 영화 드라마 추천 포스팅 마칩니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이 페이지를 즐겨찾기 하시고 필요하실 때마다 찾아보시길 바랍니다.</p>'
    },
    {
      type: 'body',
      html: '<p>도움이 되었길 바랍니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
