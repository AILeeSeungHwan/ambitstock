const post = {
  id: 158,
  sections: [
    {
      type: 'intro',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>2024년 12월 04일 개봉영화 \'소방관\' 시사회 관람평 리뷰입니다.</b></span></p>'
    },
    {
      type: 'image',
      src: '/images/post158_thumb.svg',
      alt: '현재상영작 소방관 시사회 관람평 리뷰 포토 < 홍제동 화재 실화기반 영화 >',
      caption: '현재상영작 소방관 시사회 관람평 리뷰 포토 < 홍제동 화재 실화기반 영화 >'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post158_img1.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/horror-movie-recommend-guide/',
      text: '2024년 공포 영화 추천 인기 순위 총정리 현재상영작'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '소방관 시사회 반응 출연진 발언',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><b>11월 25일 CGV 용산점에서 언론 및 관계자들을 대상으로 한 시사회</b>가 열렸습니다.</p>'
    },
    { type: 'image', src: '/images/post158_img2.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_poster.jpg', alt: '포스트 158 관련 이미지', caption: '출처: 네이버 영화' },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>2001년 홍제동 화재 방화 사건을 모티브</b></span>로 하여 소방관들의 희생과 노고를 진정성 있게 그려냈다는 평을 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>구급대원 서희 역의 이유영</b>은 <b>“실화라 생각하니 가슴이 아팠다”</b>라며, 출연을 통해 소방관들의 어려움을 널리 알리고 싶었다고 밝혔습니다. 또한, 출동 상황에서 가족들이 느끼는 불안감을 연기하며 깊은 감명을 받았다고 전했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>배우 장영남</b>은 <b>“영화 속 인물을 보며 가슴이 먹먹해져 많이 울었다”</b>라며 소방관들의 헌신에 감사와 존경을 표했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>곽경택 감독</b>은 영화를 준비하면서 <span style=&quot;background-color: #9feec3;&quot;><b>소방관들에게 얼마나 현실적으로 다가갈 수 있을지 고민</b></span>했다고 언급하며, 영화가 그들의 이야기를 제대로 담았다고 인정받기를 바랐습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '소방관 시사회 긍정적 평가 리뷰',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>영화 \'소방관\' 시사회 후기는 대부분 긍정적인 평가가 주를 이뤘습니다. </b></span></p>'
    },
    { type: 'image', src: '/images/post158_img11.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img10.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img9.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img8.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img7.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img6.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img5.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img4.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post158_img3.jpg', alt: '소방관', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<h4><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>영화를 보는 내내<span> </span>소방관들의 희생정신이<span> </span>얼마나 대단한지 새삼 느꼈다</span></b></h4>'
    },
    {
      type: 'body',
      html: '<p>실화를 바탕으로 한 영화의 특성상 소방관들의 희생과 용기를 진정성 있게 다룬 점이 관객들의 큰 감동을 자아냈습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>배우들의 몰입감 있는 연기와 실화를 재현한 연출은 관객들로 하여금 큰 공감을 불러일으켰습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>실화를 바탕</b>으로 한 만큼, 관객들은 영화 내내 울컥하는 순간들이 많았다는 평이 이어졌습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>영화 속 소방관들의 헌신과 가족들의 고통이 관객들에게 깊은 여운을 남겼다고 전해집니다.</p>'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>“영화를 보는 내내 <br>소방관들의 희생정신이 <br>얼마나 대단한지 새삼 느꼈다”</br></br></span></b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<h4 style=&quot;color: inherit; text-align: start;&quot;><b>사이렌 소리만 들어도 마음이 철렁했다</b></h4>'
    },
    {
      type: 'body',
      html: '<p>시사회에서 배우들의 연기력 측면에서도 극찬을 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>주원, 이유영, 장영남 등 출연 배우들의 연기가 특히 호평을 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>이유영은 “사이렌 소리만 들어도 마음이 철렁했다”라고 전하며 작품에 대한 열의</b>를 드러냈고, 그 <span style=&quot;background-color: #9feec3;&quot;><b>진정성이 관객들에게도 전달되었다는 반응</b></span>입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>주원 역시 영화의 진정성을 살리기 위해 섬세한 감정을 잘 표현했다는 평가를 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>소방관들이 자신의 이야기라고 느낄 수 있는 영화를 만들고 싶었다</b></h4>'
    },
    {
      type: 'body',
      html: '<p>곽경택 감독은 <b><span style=&quot;background-color: #9feec3;&quot;>“소방관들이 자신의 이야기라고 느낄 수 있는 영화를 만들고 싶었다”라고</span></b> 밝히며, 실제 소방관들과 현장의 모습을 충실히 재현하기 위해 노력했다고 전했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>이러한 진심 어린 접근은 관객들에게도 전달되어 <span style=&quot;background-color: #9feec3;&quot;><b>“영화가 단순한 극적 연출을 넘어 진정성을 느끼게 했다”</b></span>는 평가를 받았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>또한, 영화가 관객들에게 소방관의 고충과 희생을 더욱 깊이 이해하게 했다는 점이 주요 반응 중 하나로 꼽혔습니다.</p>'
    },
    {
      type: 'body',
      html: '<h4><b>소방관들의 노고를 당연하게 여겼던 것 같아 반성했다</b></h4>'
    },
    {
      type: 'body',
      html: '<p>관객들은 영화가 단순히 감동을 주는 데 그치지 않고, 소방관들의 헌신과 희생을 생각하게 만들었다고 말했습니다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>“영화를 보면서 소방관들의 노고를 당연하게 여겼던 것 같아 반성했다”</b>는 후기와 함께, <span style=&quot;background-color: #9feec3;&quot;><b>소<span style=&quot;background-color: #9feec3;&quot;>방</span>관이라는 직업이 가지는 위험성과 사회적 중요성에 대해 다시금 깨달았다는 의견</b></span>이 많았습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>영화 \'소방관\'은 12월 4일 개봉하였으며, 시사회 후반부에서도 관객들과 관계자들이 큰 박수를 보냈습니다.</p>'
    },
    {
      type: 'ending',
      html: '<p><b>12월의 최고 기대작 영화 소방관, 영화관에서 확인해 보시길 바랍니다.</b></p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
