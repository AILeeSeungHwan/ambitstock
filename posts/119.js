const post = {
  id: 119,
  sections: [
    {
      type: 'intro',
      html: '<p>대도시의 사랑법 넷플릭스 OTT 공개 < 해외 반응 관람평점 후기 ></p>'
    },
    {
      type: 'image',
      src: '/images/post119_thumb.svg',
      alt: '대도시의 사랑법 넷플릭스 OTT 공개 < 해외 반응 관람평점 후기 >',
      caption: '대도시의 사랑법 넷플릭스 OTT 공개 < 해외 반응 관람평점 후기 >'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '영화 대도시의 사랑법 넷플릭스 공개',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>영화 대도시의 사랑법이 2024년 12월 30일 넷플릭스에 공개되었습니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>2024년 10월 01일 개봉한 영화로 <span style=&quot;background-color: #9feec3;&quot;><b>3개월 정도 되는 시점에 OTT에 공개</b></span>되며, 디즈니 플러스와 마찬가지로 개봉 후 3개월 OTT 공식이 이어지는 모습입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>어떤 점에선 개봉 후 금방 OTT에 올라오는 현상때문에, &quot;꼭 보고 싶은 영화, 꼭 영화관에서 시청해야만 온전히 즐길 수 있는 영화&quot;가 아니라면 <b>영화관을 찾지 않게 되는 악순환</b>이 생기게 되는것 같지만(실제로 해당 영화는 80만의 관객수에 그쳤습니다.), 역시나 모든 해결법은 <span style=&quot;background-color: #9feec3;&quot;><b>볼만한 영화를 만드는 것</b></span>, 그리고 <span style=&quot;background-color: #9feec3;&quot;><b>너무나도 비싸진 영화표를 조정하는 것</b></span>이 근본적인 해결법이라고 생각합니다.</p>'
    },
    {
      type: 'body',
      html: '<p>그렇다면 이번 포스팅에서는 <span style=&quot;background-color: #9feec3;&quot;><b>대도시의 사랑법의 해외 시청자 반응과 관람평점</b></span>을 함께 살펴보도록 하겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    {
      type: 'cta',
      href: '/movie-drama-recommend-netflix/',
      text: '볼만한 영화 드라마 추천 넷플릭스 시청가능 <이동진 박평식 한줄평 추천작품>'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/movie-recommend-critic-masterpiece/',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '대도시의 사랑법 줄거리',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<blockquote><span style=&quot;color: #781b33;&quot;><b><span style=&quot;font-family: \'Noto Serif KR\';&quot;>미친X과 게이가 만났다! <br>바야흐로 애니멀 라이프가 <br>시작된 것이다.</br></br></span> </b></span></blockquote>'
    },
    {
      type: 'body',
      html: '<p>시선을 사로잡는 과감한 스타일과 남의 시선을 의식하지 않는 당당한 애티튜드로 모두의 관심을 끄는 자유로운 영혼, 재희. 그런 재희가 특별히 관심을 두지 않던 흥수에게 위기가 찾아온다.</p>'
    },
    {
      type: 'body',
      html: '<p>그 누구에게도 절대 들키고 싶지 않은 비밀이 하필 재희에게 들켜버린 것!</p>'
    },
    {
      type: 'body',
      html: '<p>그러나 그 과정에서 재희와 흥수는 서로가 이상형이 아니더라도,</p>'
    },
    {
      type: 'body',
      html: '<p>오직 두 사람만이 이해할 수 있는 순간이 있다는 사실을 알게 된다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>남들이 만들어낸 수많은 소문을 뒤로하고, 재희와 흥수는 사랑도 인생도 자기답게!</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>의기투합하여 동거를 시작한다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>청춘들의 달콤한 사랑과 이별의 아픔.</p>'
    },
    {
      type: 'body',
      html: '<p>어딘가 모르게 나와 비슷한 사랑의 경험을 한 고영의 성장기, 코미디와 정통 멜로, 로코가 어우러진 발칙한 멜로 드라마!</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '대도시의 사랑법 (Love In the Big City) 영어 제목',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>영화 대도시의 사랑법의 영어제목은 Love In the Big City 입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>한국어 제목과 동일한 제목을 채택한 것을 알 수 있습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section5',
      text: '대도시의 사랑법 해외 평점',
      gradientStyle: 'linear-gradient(to right, #00897b, #26a69a)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: #333333; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>대도시의 사랑법 해외 평점입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>해외 시청자 평점 7.6점으로 높은 평점을 기록하였습니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>IMDB에서의 평점은 7.5점이 넘어가면 상위권이라 할 수 있을 정도로 낮은 평점이 평균점인데, 상당히 높은 평점을 기록하였습니다.</p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분이 해외 시청자들에게 좋은 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    {
      type: 'h2',
      id: 'section6',
      text: '대도시의 사랑법 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #1a1a2e, #e94560)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>대도시의 사랑법 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p><figure class=&quot;imageblock widthContent&quot; data-origin-height=&quot;808&quot; data-origin-width=&quot;565&quot;><span data-alt=&quot;대도시의 사랑법 해외 결말후기 관람평&quot; data-phocus=&quot;https://blog.kakaocdn.net/dna/cdVHds/btsLAwUeGZo/AAAAAAAAAAAAAAAAAAAAANq8On1E5qMzV5wVQordwORtFOPFRU6AUkjWCpvQINBS/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=flrJwUNJusyC98DnJzUDpUs47P4%3D&quot; data-url=&quot;https://blog.kakaocdn.net/dna/cdVHds/btsLAwUeGZo/AAAAAAAAAAAAAAAAAAAAANq8On1E5qMzV5wVQordwORtFOPFRU6AUkjWCpvQINBS/img.png?credential=yqXZFxpELC7KVnFOS48ylbz2pIh7yKj8&amp;expires=1774969199&amp;allow_ip=&amp;allow_referer=&amp;signature=flrJwUNJusyC98DnJzUDpUs47P4%3D&quot;></span><figcaption>대도시의 사랑법 해외 결말후기 관람평</figcaption> </figure> </p>'
    },
    {
      type: 'body',
      html: '<p><b>Jericho Tadeo - 평점 4.5/5 (오리지널 스코어)</b><br/>대도시의 사랑법은 반란의 힘, 공동체의 중요성, 그리고 가장 <span style=&quot;background-color: #9feec3;&quot;><b>근본적인 차원에서 잘 만들어진 영화를 보는 순수한 기쁨</b></span>을 다시 한번 일깨워준다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>James Marsh - 평점 5/5 (오리지널 스코어)</b><br/>대도시의 사랑법에서 나온 것은 단순히 <span style=&quot;background-color: #9feec3;&quot;><b>최근 기억에 남는 가장 지능적이고 통찰력 있는 로맨틱 코미디</b></span>일 뿐만 아니라, <span style=&quot;background-color: #9feec3;&quot;><b>올해 가장 뛰어나고 볼만한 가치가 있는 영화</b></span>이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Pierce Conran - 평점 없음</b><br/>때로는 예기치 않게 등장하는 영화가 있다.</p>'
    },
    {
      type: 'body',
      html: '<p>대도시의 사랑법은 바로 그런 영화로, 한국 영화의 풍경을 풍요롭게 만들어주는 작품이다.</p>'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>올해는 저조한 영화들이 조용히 다시 등장하는 해였지만, 이 영화는 그 중에서 빛을 발한다.</b></span></p>'
    },
    {
      type: 'h2',
      id: 'section7',
      text: '대도시의 사랑법 해외 시청자 관람평',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'body',
      html: '<p><b>평점 2.5/5</b><br/>그다지 나쁘지는 않지만, 내가 가장 좋아하는 영화는 아니다. 그저 라임 같은 느낌이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>평점 4/5</b><br/>의미 있고, 사랑스럽고, 친절하다 💖</p>'
    },
    {
      type: 'body',
      html: '<p><b>평점 5/5</b><br/>정말 아름다운 영화였다! 나는 아무것도 모르고 이 영화를 보러 갔고, 영화관을 나설 때는 눈물이 흘렀다. 이 영화는 정말 중요한 영화이고, 꼭 필요한 변화를 가져올 수 있기를 진심으로 바란다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>평점 4/5</b><br/>이 영화는 어머니와 함께 커밍아웃 전에 보기 딱 좋은 영화일 것 같다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>평점 5/5</b><br/>성장과 우정에 관한 아름다운 이야기. 간단하고 직설적인 대본이 두 명의 뛰어난 주연 배우들과 조연들의 연기로 살아난다. 그들의 이야기는 나를 울게 하고, 웃게 하고, 멜랑콜리하게 만든다. 반드시 봐야 할 성장 영화. 최고의 영화 중 최고다.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 영화 대도시의 사랑법 해외 평점과 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
