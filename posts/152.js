const post = {
  id: 152,
  sections: [
    {
      type: 'intro',
      html: '<p>그해 크리스마스에는 넷플릭스 애니메이션 영화 평점 관람평 정보 < 겨울 영화 추천 feat 애드 시런 ost></p>'
    },
    {
      type: 'image',
      src: '/images/post152_thumb.svg',
      alt: '그해 크리스마스에는 넷플릭스 애니메이션 영화 평점 관람평 정보 < 겨울 영화 추천 feat 애드 시런 ost>',
      caption: '그해 크리스마스에는 넷플릭스 애니메이션 영화 평점 관람평 정보 < 겨울 영화 추천 feat 애드 시런 ost>'
    },
    { type: 'toc' },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section1',
      text: '넷플릭스 애니메이션 영화 그해 크리스마스에는 평점 관람평 정보',
      gradientStyle: 'linear-gradient(to right, #e50914, #ff6b6b)'
    },
    {
      type: 'h2',
      id: 'section2',
      text: '함께 보면 좋은 콘텐츠',
      gradientStyle: 'linear-gradient(to right, #1a73e8, #42a5f5)'
    },
    {
      type: 'body',
      html: '<p><b>더 많은 포스팅을 통해 볼만한 영화를 찾아보세요.</b></p>'
    },
    { type: 'image', src: '/images/post152_img1.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    {
      type: 'cta',
      href: '/movie-recommend-overseas-critic-rating-guide/',
      text: '크리스토퍼 놀란 영화 추천 | 모든 작품 로튼 토마토 평점 인기 순위 총정리!'
    },
    {
      type: 'cta',
      href: '/critic-recommend-masterpiece-movie-top-10/',
      text: '이동진 평론가가 추천하는 별 다섯개 명작! 볼만한 영화 추천 TOP 10'
    },
    {
      type: 'cta',
      href: '/movie-recommend-rating-top-9/',
      text: '영화 추천 쿠엔틴타란티노 필모그래피 수상작품 평점 인기 순위 TOP 9'
    },
    {
      type: 'cta',
      href: '/movie-recommend-critic-masterpiece/',
      text: '박평식 평론가 평점 9점! 2024년에도 진한 명작 영화 추천 순위 Top 10'
    },
    {
      type: 'h2',
      id: 'section3',
      text: '그해 크리스마스에는 로튼 토마토 평론가 평점',
      gradientStyle: 'linear-gradient(to right, #6a1b9a, #ab47bc)'
    },
    {
      type: 'body',
      html: '<p style=&quot;color: inherit; text-align: start;&quot;><span style=&quot;background-color: #f3c000;&quot;><b>그해 크리스마스에는 로튼 토마토 평론가 평점입니다.</b></span><span style=&quot;background-color: #f3c000;&quot;></span></p>'
    },
    { type: 'image', src: '/images/post152_img2.jpg', alt: '그해 크리스마스에는 넷플릭스 애니메이션 영화 평점 관람평 정보 < 겨울 영화 추천 feat 애드 시런 ost>', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #9feec3;&quot;><b>평론가 평점 64%, 관객 평점은 74%를 기록하였습니다.</b></span></p>'
    },
    {
      type: 'body',
      html: '<p>평론가와 관객 모두 높은 평점을 기록하지는 못하였으나, 한국에서는 평점 8점 후반대로 굉장히 높은 점수를 기록중입니다.</p>'
    },
    {
      type: 'body',
      html: '<p>어떠한 부분의 평론가들과 해외 관객들에게 좋지 못한 평점을 이끌었는지 후기 관람평 확인해 보겠습니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
    {
      type: 'h2',
      id: 'section4',
      text: '그해 크리스마스에는 평론가 결말후기 관람평',
      gradientStyle: 'linear-gradient(to right, #e53935, #ef5350)'
    },
    {
      type: 'body',
      html: '<p><span style=&quot;background-color: #f3c000;&quot;><b>그해 크리스마스에는 평론가 결말후기 관람평입니다.</b></span></p>'
    },
    { type: 'image', src: '/images/post152_img9.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post152_img8.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post152_img7.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post152_img6.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post152_img5.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post152_img4.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    { type: 'image', src: '/images/post152_img3.jpg', alt: '그해 크리스마스에는', caption: 'ⓒ R의 필름공장' },
    {
      type: 'body',
      html: '<p><b>Alan French - 평점 없음</b><br>애니메이션 영화에서 충분한 감정적인 카타르시스를 제공한 끝에, 그해 크리스마스에는 <b>어린이와 청소년들을 위한 크리스마스 이야기의 좋은 입문</b>이 될 수 있을 것이다.</br></p>'
    },
    {
      type: 'body',
      html: '<p><b>Lynn Venhaus - 평점 B+</b><br/>매력적이고 기분 좋은 &quot;그해 크리스마스에는&quot;은 너무 달콤하고 비현실적인 상황들로 인해 많은 휴일 테마 영화들이 흔히 탈선하는 그런 요소 없이 <b>따뜻한 감정을 안겨주는 애니메이션 코미디</b>이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Brian Viner - 평점 3/5</b><br/>영화는 꽤 즐겁게 흘러간다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Mat Brunet - 평점 5/10</b><br/>그해 크리스마스에는 나쁘지 않았지만, 이 영화를 기억할 것 같지는 않다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Sara Wallis - 평점 4/5</b><br/>&quot;그해 크리스마스에는&quot;의 이야기 속도는 부족할 수 있지만, 훌륭한 캐스트 리스트가 이를 보완하며, <b>에드 시런의 새 노래 &quot;Under The Tree&quot;도 포함</b>되어 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Rich Cline - 평점 3/5</b><br/>영화가 휴일의 공식에서 벗어나지 않기 때문에, <b>눈 오는 오후에 따뜻한 초콜릿 한 잔처럼 편안함</b>을 느낄 수 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Helen O\'Hara - 평점 3/5</b><br/>각각의 이야기는 개별적으로 매력적이지만, 지나치게 익숙한 애니메이션과 부족한 캐릭터 디자인이 그 효과를 둔화시킨다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Stefan Pape - 평점 없음</b><br/>이 달콤한 애니메이션은 &quot;러브 액츄얼리&quot;의 리차드 커티스의 손에서 나온 매력적인 연휴 간식이다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Rachel Wagner - 평점 없음</b><br/>작고 달콤한 크리스마스 영화. 리차드 커티스가 다시 한 번 멋진 작품을 선보였다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Charles Koplinski - 평점 1.5/4.0</b><br/>세 개의 다른 영국 아동 도서를 바탕으로 한 시몬 오토의 애니메이션 영화 &quot;그해 크리스마스에는&quot;은 휴일 감성을 제대로 전달하려 하지만, <b>결국 지루한 영화</b>가 되어버린다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Ferdosa Abdi - 평점 8/10</b><br/>이 영화의 장점은 다이내믹한 목소리 출연진이 이 해변 마을과 이야기에 생동감과 활력을 더해주는 데 있다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Casey Chong - 평점 4/5</b><br/>익숙한 이 애니메이션 크리스마스 이야기는 산타의 착한 아이 목록에 들어갈 만하다.</p>'
    },
    {
      type: 'body',
      html: '<p><b>Luna Guthrie - 평점 4/10</b><br/>그해 크리스마스에는 매력과 혁신을 추구했지만, 결국 예측 가능하고 다소 실망스러운 영화가 되어 어린 관객층을 겨냥한 듯한 느낌을 준다.</p>'
    },
    {
      type: 'body',
      html: '<p>이상으로 넷플릭스 애니메이션 영화 그해 크리스마스에는 로튼 토마토 해외 평론가 관람평 결말후기 포스팅 마칩니다.</p>'
    },
    {
      type: 'ending',
      html: '<p>감사합니다.</p>'
    },
    { type: 'ad', slot: '6297515693', format: 'auto' },
  ]
}

module.exports = post
